...
public class RecoveryUploadService extends IntentService {
    ...
    public static synchronized void startAlarm(Context context, String str) {
        synchronized (RecoveryUploadService.class) {
            if (!Util.isNullOrNil(str)) {
                CommonOptions iz = OptionFactory.iz(context);
                ...
                RecoveryLog.i(TAG, "startAlarm %s lastUploadTime %s", str, Util.nz(j));
                ...
                if (service == null || currentTimeMillis - j > iz.Arr) {
                    ...
                    RecoveryLog.i(TAG, "startAlarm pendingIntent success: %d will report %s", Integer.valueOf(r2.hashCode()), Util.nz(j));
                } else {
    ...
    public static void startAlarmAfter(Context context, String str, long j) {
        if (!Util.isNullOrNil(str)) {
            ...
            RecoveryLog.i(TAG, "startAlarmAfter pendingIntent:%d %s", Integer.valueOf(service.hashCode()), Util.nz(r2));
        }
    }
    ...
    public void onHandleIntent(Intent intent) {
        ...
        if (!tryToUploadData) {
            startAlarmAfter(this, stringExtra, OptionFactory.iz(this).Arr);
        }
