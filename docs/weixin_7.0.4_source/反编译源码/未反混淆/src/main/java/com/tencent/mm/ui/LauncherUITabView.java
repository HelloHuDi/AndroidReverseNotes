package com.tencent.mm.ui;

import android.content.Context;
import android.graphics.Bitmap;
import android.graphics.Bitmap.Config;
import android.graphics.Canvas;
import android.graphics.Matrix;
import android.os.Message;
import android.util.AttributeSet;
import android.view.View;
import android.view.View.OnClickListener;
import android.widget.ImageView;
import android.widget.ImageView.ScaleType;
import android.widget.LinearLayout;
import android.widget.LinearLayout.LayoutParams;
import android.widget.RelativeLayout;
import com.tencent.matrix.trace.core.AppMethodBeat;
import com.tencent.mm.R;
import com.tencent.mm.g.a.js;
import com.tencent.mm.sdk.platformtools.ab;
import com.tencent.mm.sdk.platformtools.ak;

public class LauncherUITabView extends RelativeLayout implements c {
    private Matrix aKk = new Matrix();
    protected int lWS = 0;
    private long nlj = 0;
    private com.tencent.mm.ui.c.a yjX;
    private int ykG;
    private Bitmap ykH;
    private ImageView ykI;
    protected a ykJ;
    protected a ykK;
    protected a ykL;
    protected a ykM;
    private int ykn = -1;
    protected OnClickListener ykp = new OnClickListener() {
        private final long ryi = 300;

        public final void onClick(View view) {
            AppMethodBeat.i(29515);
            int intValue = ((Integer) view.getTag()).intValue();
            if (LauncherUITabView.this.ykn == intValue && intValue == 0 && System.currentTimeMillis() - LauncherUITabView.this.nlj <= 300) {
                ab.v("MicroMsg.LauncherUITabView", "onMainTabDoubleClick");
                LauncherUITabView.this.ykq.removeMessages(0);
                com.tencent.mm.sdk.b.a.xxA.m(new js());
                LauncherUITabView.this.nlj = System.currentTimeMillis();
                LauncherUITabView.this.ykn = intValue;
                AppMethodBeat.o(29515);
                return;
            }
            if (LauncherUITabView.this.yjX != null) {
                if (intValue == 0 && LauncherUITabView.this.ykn == 0) {
                    LauncherUITabView.this.ykq.sendEmptyMessageDelayed(0, 300);
                } else {
                    LauncherUITabView.this.nlj = System.currentTimeMillis();
                    LauncherUITabView.this.ykn = intValue;
                    LauncherUITabView.this.yjX.onTabClick(intValue);
                    AppMethodBeat.o(29515);
                    return;
                }
            }
            LauncherUITabView.this.nlj = System.currentTimeMillis();
            LauncherUITabView.this.ykn = intValue;
            ab.w("MicroMsg.LauncherUITabView", "on tab click, index %d, but listener is null", (Integer) view.getTag());
            AppMethodBeat.o(29515);
        }
    };
    private ak ykq = new ak() {
        public final void handleMessage(Message message) {
            AppMethodBeat.i(29516);
            ab.v("MicroMsg.LauncherUITabView", "onMainTabClick");
            LauncherUITabView.this.yjX.onTabClick(0);
            AppMethodBeat.o(29516);
        }
    };
    private int ykr = 0;
    private int yks = 0;
    private int ykt = 0;
    private boolean yku = false;
    private int ykw = 0;
    private boolean ykx = false;

    protected class a {
        MMTabView ykO;

        protected a() {
        }
    }

    public LauncherUITabView(Context context, AttributeSet attributeSet) {
        super(context, attributeSet);
        AppMethodBeat.i(29517);
        init();
        AppMethodBeat.o(29517);
    }

    public LauncherUITabView(Context context, AttributeSet attributeSet, int i) {
        super(context, attributeSet, i);
        AppMethodBeat.i(29518);
        init();
        AppMethodBeat.o(29518);
    }

    public void setOnTabClickListener(com.tencent.mm.ui.c.a aVar) {
        this.yjX = aVar;
    }

    private a MX(int i) {
        AppMethodBeat.i(29519);
        a aVar = new a();
        aVar.ykO = new MMTabView(getContext(), i);
        aVar.ykO.setTag(Integer.valueOf(i));
        aVar.ykO.setOnClickListener(this.ykp);
        AppMethodBeat.o(29519);
        return aVar;
    }

    private a f(LinearLayout linearLayout) {
        AppMethodBeat.i(29520);
        a MX = MX(0);
        MX.ykO.setText((int) R.string.cv9);
        LayoutParams layoutParams = new LayoutParams(0, getResources().getDimensionPixelSize(R.dimen.a_s));
        layoutParams.weight = 1.0f;
        linearLayout.addView(MX.ykO, layoutParams);
        AppMethodBeat.o(29520);
        return MX;
    }

    private a g(LinearLayout linearLayout) {
        AppMethodBeat.i(29521);
        a MX = MX(1);
        MX.ykO.setText((int) R.string.cv_);
        LayoutParams layoutParams = new LayoutParams(0, getResources().getDimensionPixelSize(R.dimen.a_s));
        layoutParams.weight = 1.0f;
        linearLayout.addView(MX.ykO, layoutParams);
        AppMethodBeat.o(29521);
        return MX;
    }

    private a h(LinearLayout linearLayout) {
        AppMethodBeat.i(29522);
        a MX = MX(2);
        MX.ykO.setText((int) R.string.cv8);
        LayoutParams layoutParams = new LayoutParams(0, getResources().getDimensionPixelSize(R.dimen.a_s));
        layoutParams.weight = 1.0f;
        linearLayout.addView(MX.ykO, layoutParams);
        AppMethodBeat.o(29522);
        return MX;
    }

    private a i(LinearLayout linearLayout) {
        AppMethodBeat.i(29523);
        a MX = MX(3);
        MX.ykO.setText((int) R.string.cw4);
        LayoutParams layoutParams = new LayoutParams(0, getResources().getDimensionPixelSize(R.dimen.a_s));
        layoutParams.weight = 1.0f;
        linearLayout.addView(MX.ykO, layoutParams);
        AppMethodBeat.o(29523);
        return MX;
    }

    private void dxN() {
        AppMethodBeat.i(29524);
        this.ykI = new ImageView(getContext());
        this.ykI.setImageMatrix(this.aKk);
        this.ykI.setScaleType(ScaleType.MATRIX);
        this.ykI.setId(2307142);
        RelativeLayout.LayoutParams layoutParams = new RelativeLayout.LayoutParams(-1, com.tencent.mm.bz.a.fromDPToPix(getContext(), 3));
        layoutParams.addRule(8, 2307141);
        addView(this.ykI, layoutParams);
        AppMethodBeat.o(29524);
    }

    private void init() {
        AppMethodBeat.i(29525);
        LinearLayout linearLayout = new LinearLayout(getContext());
        linearLayout.setBackgroundResource(R.color.a69);
        linearLayout.setId(2307141);
        linearLayout.setOrientation(0);
        addView(linearLayout, new RelativeLayout.LayoutParams(-1, -2));
        dxN();
        this.ykJ = f(linearLayout);
        this.ykK = g(linearLayout);
        this.ykL = h(linearLayout);
        this.ykM = i(linearLayout);
        AppMethodBeat.o(29525);
    }

    /* Access modifiers changed, original: protected */
    public void onLayout(boolean z, int i, int i2, int i3, int i4) {
        AppMethodBeat.i(29526);
        super.onLayout(z, i, i2, i3, i4);
        ab.d("MicroMsg.LauncherUITabView", "on layout, width %d", Integer.valueOf(i3 - i));
        this.ykG = (i3 - i) / 4;
        int i5 = this.ykG;
        if (this.ykH == null || this.ykH.getWidth() != i5) {
            String str = "MicroMsg.LauncherUITabView";
            String str2 = "sharp width changed, from %d to %d";
            Object[] objArr = new Object[2];
            objArr[0] = Integer.valueOf(this.ykH == null ? -1 : this.ykH.getWidth());
            objArr[1] = Integer.valueOf(i5);
            ab.w(str, str2, objArr);
            this.ykH = Bitmap.createBitmap(i5, com.tencent.mm.bz.a.fromDPToPix(getContext(), 3), Config.ARGB_8888);
            new Canvas(this.ykH).drawColor(getResources().getColor(R.color.a61));
            l(this.lWS, 0.0f);
            this.ykI.setImageBitmap(this.ykH);
        }
        setTo(this.lWS);
        AppMethodBeat.o(29526);
    }

    public final void MM(int i) {
        AppMethodBeat.i(29527);
        ab.d("MicroMsg.LauncherUITabView", "updateMainTabUnread %d", Integer.valueOf(i));
        this.ykr = i;
        if (i <= 0) {
            this.ykJ.ykO.setUnread(null);
            AppMethodBeat.o(29527);
        } else if (i > 99) {
            this.ykJ.ykO.setUnread(getContext().getString(R.string.eyo));
            AppMethodBeat.o(29527);
        } else {
            this.ykJ.ykO.setUnread(String.valueOf(i));
            AppMethodBeat.o(29527);
        }
    }

    public final void dwp() {
        AppMethodBeat.i(29528);
        if (this.ykJ == null || this.ykK == null || this.ykL == null || this.ykM == null) {
            AppMethodBeat.o(29528);
            return;
        }
        this.ykJ.ykO.dyw();
        this.ykK.ykO.dyw();
        this.ykL.ykO.dyw();
        this.ykM.ykO.dyw();
        AppMethodBeat.o(29528);
    }

    public final void MN(int i) {
        AppMethodBeat.i(29529);
        this.yks = i;
        if (i <= 0) {
            this.ykK.ykO.setUnread(null);
            AppMethodBeat.o(29529);
        } else if (i > 99) {
            this.ykK.ykO.setUnread(getContext().getString(R.string.eyo));
            AppMethodBeat.o(29529);
        } else {
            this.ykK.ykO.setUnread(String.valueOf(i));
            AppMethodBeat.o(29529);
        }
    }

    public final void MO(int i) {
        AppMethodBeat.i(29530);
        this.ykt = i;
        if (i <= 0) {
            this.ykL.ykO.setUnread(null);
            AppMethodBeat.o(29530);
        } else if (i > 99) {
            this.ykL.ykO.setUnread(getContext().getString(R.string.eyo));
            AppMethodBeat.o(29530);
        } else {
            this.ykL.ykO.setUnread(String.valueOf(i));
            AppMethodBeat.o(29530);
        }
    }

    public final void MP(int i) {
        AppMethodBeat.i(29531);
        this.ykw = i;
        if (i <= 0) {
            this.ykM.ykO.setUnread(null);
            AppMethodBeat.o(29531);
        } else if (i > 99) {
            this.ykM.ykO.setUnread(getContext().getString(R.string.eyo));
            AppMethodBeat.o(29531);
        } else {
            this.ykM.ykO.setUnread(String.valueOf(i));
            AppMethodBeat.o(29531);
        }
    }

    public final void px(boolean z) {
        AppMethodBeat.i(29532);
        this.yku = z;
        this.ykL.ykO.pR(z);
        AppMethodBeat.o(29532);
    }

    public final void py(boolean z) {
        AppMethodBeat.i(29533);
        this.ykx = z;
        this.ykM.ykO.pR(z);
        AppMethodBeat.o(29533);
    }

    public final void pz(boolean z) {
    }

    public final void l(int i, float f) {
        AppMethodBeat.i(29534);
        this.aKk.setTranslate(((float) this.ykG) * (((float) i) + f), 0.0f);
        this.ykI.setImageMatrix(this.aKk);
        AppMethodBeat.o(29534);
    }

    public int getCurIdx() {
        return this.lWS;
    }

    public void setTo(int i) {
        AppMethodBeat.i(29535);
        this.lWS = i;
        this.ykJ.ykO.setTextColor(i == 0 ? getResources().getColorStateList(R.color.a61) : getResources().getColorStateList(R.color.a7k));
        this.ykK.ykO.setTextColor(i == 1 ? getResources().getColorStateList(R.color.a61) : getResources().getColorStateList(R.color.a7k));
        this.ykL.ykO.setTextColor(i == 2 ? getResources().getColorStateList(R.color.a61) : getResources().getColorStateList(R.color.a7k));
        this.ykM.ykO.setTextColor(i == 3 ? getResources().getColorStateList(R.color.a61) : getResources().getColorStateList(R.color.a7k));
        this.nlj = System.currentTimeMillis();
        this.ykn = this.lWS;
        AppMethodBeat.o(29535);
    }

    public int getMainTabUnread() {
        return this.ykr;
    }

    public int getContactTabUnread() {
        return this.yks;
    }

    public int getFriendTabUnread() {
        return this.ykt;
    }

    public int getSettingsTabUnread() {
        return this.ykw;
    }

    public boolean getShowFriendPoint() {
        return this.yku;
    }

    public boolean getSettingsPoint() {
        return this.ykx;
    }
}
