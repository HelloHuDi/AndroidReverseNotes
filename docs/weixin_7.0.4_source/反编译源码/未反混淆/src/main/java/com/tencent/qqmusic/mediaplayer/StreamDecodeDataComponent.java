...
public class StreamDecodeDataComponent extends BaseDecodeDataComponent {
        ...
        try {
            ...
            while (!this.mCorePlayer.mIsExit) {
                ...
                synchronized (this.mSeekRecord) {
                    try {
                        if (!this.mSeekRecord.empty()) {
                            ...
                            Logger.i(TAG, "execute seek: " + i5 + ", abandon the others");
                        }
                    } finally {
                        while (true) {
                        }
                ...
                if (i5 < 0 && this.mNeedFlush) {
                    ...
                    Logger.i(TAG, axiliary("[run] flashback to ".concat(String.valueOf(i5))));
                ...
                if (!isPaused()) {
                    if (isIdle() || isError() || isStopped()) {
                        ...
                        Logger.i(TAG, axiliary("writeShortCount: " + i6 + ", playFramePosition: " + i4 + ", leftTimeInMs: " + i5 + ", minAudioTrackWaitTimeMs: " + i3));
                        if (i5 > 0) {
                            synchronized (this) {
                                try {
                                ...
                                if (i == 0) {
                                    ...
                                    if (calcBitMinSize > 0) {
                                        ...
                                        while (((long) this.mDecodeBufferInfo.bufferSize) < calcBitMinSize && i7 >= 0) {
                                            ...
                                            seekTo = i7 - 1;
                                            Logger.d(TAG, axiliary("decode tmpSize: " + i8 + ", mDecodeBufferInfo.bufferSize: " + this.mDecodeBufferInfo.bufferSize));
                                            i7 = seekTo;
                                        }
                                    } else {
                                        i8 = seekTo;
                                    ...
                                    if ((!this.isUseFloatInHiRes && i >= 3) || i <= 0) {
                                        Logger.w(TAG, "mPlayBitDept is set with 2, old value: ".concat(String.valueOf(i)));
                                        i = 2;
                                    }
                                    Logger.d(TAG, axiliary("mPlayBitDept: ".concat(String.valueOf(i))));
                                    seekTo = i8;
                                    j3 = j;
                                    j4 = j5;
                                } else {
                                            ...
                                            if (this.isUseFloatInHiRes) {
                                                ...
                                                if (seekTo >= 0) {
                                                    ...
                                                    if (seekTo != this.mFloatBufferInfo.bufferSize) {
                                                        Logger.w(TAG, axiliary("mAudioTrack write float not equal: " + seekTo + ", expect: " + this.mFloatBufferInfo.bufferSize));
                                                    }
                                                    i6 += this.mFloatBufferInfo.bufferSize * 2;
                                                } else if (this.mAudioTrack.getPlayState() == 3) {
                                                    Logger.e(TAG, axiliary("mAudioTrack write float failed: " + seekTo + ", expect: " + this.mFloatBufferInfo.bufferSize));
                                                    this.mStateRunner.transfer(Integer.valueOf(9));
                                                ...
                                                if (seekTo >= 0) {
                                                    ...
                                                    if (seekTo != this.mDTSBufferInfo.bufferSize) {
                                                        Logger.w(TAG, axiliary("mAudioTrack write bytes not equal: " + seekTo + ", expect: " + this.mDTSBufferInfo.bufferSize));
                                                    }
                                                    i6 += this.mDTSBufferInfo.bufferSize;
                                                } else if (this.mAudioTrack.getPlayState() == 3) {
                                                    Logger.e(TAG, axiliary("mAudioTrack write bytes failed: " + seekTo + ", expect: " + this.mDTSBufferInfo.bufferSize));
                                                    this.mStateRunner.transfer(Integer.valueOf(9));
                                                    callExceptionCallback(92, 102);
                                                    AppMethodBeat.o(104491);
                                                    return;
    ...
    private boolean createAudioTrack() {
        ...
        if (!this.mStateRunner.isEqual(Integer.valueOf(3))) {
            ...
            Logger.i(TAG, axiliary(String.format("playback_bufsize: %d, mBuffSize: %d, mPlaySample: %d, playChannel: %d, pcmEncoding: %d", new Object[]{Integer.valueOf(minBufferSize), Integer.valueOf(this.mBuffSize), Long.valueOf(this.mPlaySample), Integer.valueOf(channels), Integer.valueOf(i2)})));
            minPcmBufferSize = Math.max((int) (Math.floor((double) ((((1 * this.mPlaySample) * ((long) channels)) * 2) / ((long) minBufferSize))) + 1.0d), 1);
            Logger.d(TAG, axiliary("times: " + minPcmBufferSize + ", MIN_AUDIO_TRACK_BUFFER_TIMES: 1"));
            ...
            while (i3 > 0) {
                try {
                    ...
                    Logger.e(TAG, axiliary("audioTrack create fail!!! state = ".concat(String.valueOf(minPcmBufferSize))));
    ...
    public void setAudioStreamType(int i) {
        ...
        try {
            ...
            Logger.e(TAG, "failed in setting audio stream type to :".concat(String.valueOf(i)), e);
            AppMethodBeat.o(104496);
        }
    }
