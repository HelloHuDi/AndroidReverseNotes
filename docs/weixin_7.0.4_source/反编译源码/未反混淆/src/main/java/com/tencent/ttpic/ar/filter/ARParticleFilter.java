...
public class ARParticleFilter extends VideoFilterBase {
        ...
        public void handleMessage(Message message) {
            ...
            while (true) {
                ...
                if (i3 >= advance.size() || i3 >= ARParticleFilter.this.mSpriteList.size() || i3 >= ARParticleFilter.this.mParticleDirList.size()) {
                    ...
                    com.tencent.oscarcamera.particlesystem.FrameParticleData frameParticleData = (com.tencent.oscarcamera.particlesystem.FrameParticleData) advance.get(i3);
    ...
    public void initParams() {
        ...
        addParam(new j("u_MVPMatrix", ARMatrixUtil.getMovedMVPMatrix(this.near, this.far)));
        addParam(new i("blendMode", 0));
        addParam(new f("canvasWidth", 1.0f));
        addParam(new f("canvasHeight", 1.0f));
        addParam(new i("isFrontCamera", 0));
        ...
        while (true) {
            ...
            if (i2 >= frameData.frameParticleData.size() || i2 >= this.mParticleDirList.size()) {
                ...
                if (needCopyTex(frameParticleData.blendMode)) {
                    this.mCopyFilter.RenderProcess(hVar.texture[0], this.width, this.height, -1, 0.0d, this.mCopyFrame);
                ...
                if (changeTexture(((String) this.mParticleDirList.get(i2)) + File.separator + frameParticleData.tex)) {
                    ...
                    addParam(new i("blendMode", frameParticleData.blendMode));
                    addParam(new j("u_MVPMatrix", ARMatrixUtil.getMovedMVPMatrix(this.near, this.far)));
                    addParam(new f("canvasWidth", this.mCanvasWidth));
                    addParam(new f("canvasHeight", this.mCanvasHeight));
                    addParam(new i("isFrontCamera", ARMatrixUtil.isFrontCamera ? 1 : 0));
                    OnDrawFrameGLSL();
    ...
    private boolean changeTexture(String str) {
        ...
        if (this.mPathToBitmapIndexMapping.containsKey(str)) {
            ...
            n nVar = new n("inputImageTexture2", this.mTextures[intValue], 33986);
            nVar.initialParams(getProgramIds());
            addParam(nVar);
    ...
    public void addTouchPoint(PointF pointF) {
        ...
        point3D2.x = point3D.x - (fArr[0] * f2);
        point3D2.y = point3D.y - (fArr[1] * f2);
        point3D2.z = point3D.z - (fArr[2] * f2);
        ...
        point3D3.x = point3D.x + (fArr[0] * f2);
        point3D3.y = point3D.y + (fArr[1] * f2);
        point3D3.z = point3D.z + (fArr[2] * f2);
        Point3D point3D4 = new Point3D();
        point3D4.x = point3D.x - (fArr[0] * f2);
        point3D4.y = point3D.y - (fArr[1] * f2);
        point3D4.z = point3D.z - (f2 * fArr[2]);
        f = pointF.x / ((float) this.width);
        if (ARMatrixUtil.isFrontCamera) {
            f = 1.0f - f;
        }
        float[] fArr2 = new float[]{(point3D4.x - point3D2.x) * f, (point3D4.y - point3D2.y) * f, f * (point3D4.z - point3D2.z)};
        f = 1.0f - (pointF.y / ((float) this.height));
        float[] fArr3 = new float[]{(point3D3.x - point3D2.x) * f, (point3D3.y - point3D2.y) * f, f * (point3D3.z - point3D2.z)};
        Point3D point3D5 = new Point3D();
        point3D5.x = (point3D2.x + fArr2[0]) + fArr3[0];
        point3D5.y = (point3D2.y + fArr2[1]) + fArr3[1];
        point3D5.z = (fArr2[2] + point3D2.z) + fArr3[2];
        this.mParticleSystem.emitImmediately(point3D5.x, point3D5.y, point3D5.z);
    ...
    public void updateAndRender(h hVar) {
