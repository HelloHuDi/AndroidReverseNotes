package com.tencent.mm.protocal.protobuf;

import com.tencent.matrix.trace.core.AppMethodBeat;
import com.tencent.mm.bt.a;
import e.a.a.b;
import java.util.LinkedList;

public final class gf extends a {
    public SKBuiltinBuffer_t vIm;
    public SKBuiltinBuffer_t vIn;

    public final int op(int i, Object... objArr) {
        AppMethodBeat.i(58888);
        b bVar;
        int ix;
        if (i == 0) {
            e.a.a.c.a aVar = (e.a.a.c.a) objArr[0];
            if (this.vIm == null) {
                bVar = new b("Not all required fields were included: EncryptKey");
                AppMethodBeat.o(58888);
                throw bVar;
            } else if (this.vIn == null) {
                bVar = new b("Not all required fields were included: Key");
                AppMethodBeat.o(58888);
                throw bVar;
            } else {
                if (this.vIm != null) {
                    aVar.iy(1, this.vIm.computeSize());
                    this.vIm.writeFields(aVar);
                }
                if (this.vIn != null) {
                    aVar.iy(2, this.vIn.computeSize());
                    this.vIn.writeFields(aVar);
                }
                AppMethodBeat.o(58888);
                return 0;
            }
        } else if (i == 1) {
            if (this.vIm != null) {
                ix = e.a.a.a.ix(1, this.vIm.computeSize()) + 0;
            } else {
                ix = 0;
            }
            if (this.vIn != null) {
                ix += e.a.a.a.ix(2, this.vIn.computeSize());
            }
            AppMethodBeat.o(58888);
            return ix;
        } else if (i == 2) {
            e.a.a.a.a aVar2 = new e.a.a.a.a((byte[]) objArr[0], unknownTagHandler);
            for (ix = a.getNextFieldNumber(aVar2); ix > 0; ix = a.getNextFieldNumber(aVar2)) {
                if (!super.populateBuilderWithField(aVar2, this, ix)) {
                    aVar2.ems();
                }
            }
            if (this.vIm == null) {
                bVar = new b("Not all required fields were included: EncryptKey");
                AppMethodBeat.o(58888);
                throw bVar;
            } else if (this.vIn == null) {
                bVar = new b("Not all required fields were included: Key");
                AppMethodBeat.o(58888);
                throw bVar;
            } else {
                AppMethodBeat.o(58888);
                return 0;
            }
        } else if (i == 3) {
            e.a.a.a.a aVar3 = (e.a.a.a.a) objArr[0];
            gf gfVar = (gf) objArr[1];
            int intValue = ((Integer) objArr[2]).intValue();
            LinkedList Vh;
            int size;
            byte[] bArr;
            SKBuiltinBuffer_t sKBuiltinBuffer_t;
            e.a.a.a.a aVar4;
            boolean z;
            switch (intValue) {
                case 1:
                    Vh = aVar3.Vh(intValue);
                    size = Vh.size();
                    for (intValue = 0; intValue < size; intValue++) {
                        bArr = (byte[]) Vh.get(intValue);
                        sKBuiltinBuffer_t = new SKBuiltinBuffer_t();
                        aVar4 = new e.a.a.a.a(bArr, unknownTagHandler);
                        for (z = true; z; z = sKBuiltinBuffer_t.populateBuilderWithField(aVar4, sKBuiltinBuffer_t, a.getNextFieldNumber(aVar4))) {
                        }
                        gfVar.vIm = sKBuiltinBuffer_t;
                    }
                    AppMethodBeat.o(58888);
                    return 0;
                case 2:
                    Vh = aVar3.Vh(intValue);
                    size = Vh.size();
                    for (intValue = 0; intValue < size; intValue++) {
                        bArr = (byte[]) Vh.get(intValue);
                        sKBuiltinBuffer_t = new SKBuiltinBuffer_t();
                        aVar4 = new e.a.a.a.a(bArr, unknownTagHandler);
                        for (z = true; z; z = sKBuiltinBuffer_t.populateBuilderWithField(aVar4, sKBuiltinBuffer_t, a.getNextFieldNumber(aVar4))) {
                        }
                        gfVar.vIn = sKBuiltinBuffer_t;
                    }
                    AppMethodBeat.o(58888);
                    return 0;
                default:
                    AppMethodBeat.o(58888);
                    return -1;
            }
        } else {
            AppMethodBeat.o(58888);
            return -1;
        }
    }
}
