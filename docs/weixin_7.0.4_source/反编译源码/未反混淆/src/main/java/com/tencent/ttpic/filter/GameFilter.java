...
public class GameFilter {
    ...
    public h updateAndRender(h hVar, PTFaceAttr pTFaceAttr) {
                        ...
                        if (!this.isFirstFrame) {
                            ...
                            if (rotation == 90 || rotation == 270) {
                                if (((double) faceStatus.transform[11]) > 2.5d) {
                                    this.mFlipFilter.RenderProcess(GamePlaySDK.getInstance().getGameTexture(), this.height, this.width, -1, 0.0d, hVar);
                                }
                            } else if (((double) faceStatus.transform[11]) > 2.5d) {
                                this.mFlipFilter.RenderProcess(GamePlaySDK.getInstance().getGameTexture(), this.width, this.height, -1, 0.0d, hVar);
                            }
    ...
    public void updateActionTriggered(PTDetectInfo pTDetectInfo) {
        ...
        if (this.itemList3D != null) {
            ...
            for (StickerItem3D stickerItem3D2 : this.itemList3D) {
                ...
                LogUtils.e(TAG, String.format("item.id  = %s, item.triggered = " + stickerItem3D2.triggered, new Object[]{stickerItem3D2.id}));
            }
