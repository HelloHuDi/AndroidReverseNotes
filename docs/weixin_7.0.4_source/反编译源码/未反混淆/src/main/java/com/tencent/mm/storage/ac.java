...
public final class ac {
    public static String eSj = e.eSj;
    public static final String xIE = (ah.getContext().getFilesDir() + "/xlog");
    ...
    private static final boolean dsd() {
        ...
        Field[] declaredFields = ac.class.getDeclaredFields();
            ...
            if (Modifier.isStatic(field.getModifiers()) && ((field.getName().startsWith("USERINFO_") || field.getName().startsWith("NEW_BANDAGE_") || field.getName().startsWith("DYNAMIC_CONFIG_")) && field.getType().toString().equals("int"))) {
                try {
                    ...
                    if (sparseArray.get(i, null) != null) {
                        ab.e("MicroMsg.ConstantsStorage", "%s and %s has same value(0x%05X)!!!", sparseArray.get(i), field.getName(), Integer.valueOf(i));
                        AppMethodBeat.o(58992);
                        return false;
                    }
                    sparseArray.put(i, field.getName());
                } catch (IllegalArgumentException e) {
                    ab.e("MicroMsg.ConstantsStorage", "exception:%s", bo.l(e));
                } catch (IllegalAccessException e2) {
                    ab.e("MicroMsg.ConstantsStorage", "exception:%s", bo.l(e2));
                }
            }
        }
        if (sparseArray.size() > ErrorCode.INFO_CODE_FILEREADER_OPENFILEREADER_MINIQBFAILED) {
            ab.e("MicroMsg.ConstantsStorage", "constants values size(%d) is over the limit(%d)!!!", Integer.valueOf(sparseArray.size()), Integer.valueOf(ErrorCode.INFO_CODE_FILEREADER_OPENFILEREADER_MINIQBFAILED));
            AppMethodBeat.o(58992);
            return false;
        }
        ab.i("MicroMsg.ConstantsStorage", "checkDuplicateUserInfo values size: %d", Integer.valueOf(sparseArray.size()));
        boolean dse = dse();
        AppMethodBeat.o(58992);
        return dse;
    }
    ...
    private static final boolean dse() {
        ...
        while (i < length) {
            ...
            if (bo.isNullOrNil(aot)) {
                AppMethodBeat.o(58993);
                return false;
            } else if (arrayList.contains(aot)) {
                ab.e("MicroMsg.ConstantsStorage", "BusinessInfoKey check error: redefinition of [%s] which already defined!", aot);
                AppMethodBeat.o(58993);
                return false;
            } else {
                arrayList.add(aot);
    ...
    private static final String aot(String str) {
        ...
        while (i < split.length) {
            if (split[i] == null || split[i].length() <= 0) {
                ab.e("MicroMsg.ConstantsStorage", "BusinessInfoKey parse failed: name:%s , [%s] too short ( <1 ) ", str, split[i]);
                AppMethodBeat.o(58994);
                return null;
            }
            i++;
        ...
        ab.e("MicroMsg.ConstantsStorage", "BusinessInfoKey parse failed: name[%s], invalid type:%s ", str, str2);
        AppMethodBeat.o(58994);
        return null;
    }
}
