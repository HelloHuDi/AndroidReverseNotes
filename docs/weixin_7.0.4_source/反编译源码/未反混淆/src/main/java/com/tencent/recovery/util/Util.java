...
public class Util {
    ...
    public static final int iB(Context context) {
        ...
        i = ("A" + x(stringBuffer.toString().getBytes()).substring(0, 15)).hashCode();
        sharedPreferences.edit().putInt("KeySafeModeUUID", i).commit();
        return i;
    }

    private static final String x(byte[] bArr) {
    ...
    public static String nz(long j) {
        return new SimpleDateFormat("yyyy-MM-dd HH:mm:ss:SSS").format(new Date(j));
    }

    public static String iC(Context context) {
        return aw(context, Process.myPid());
    }

    private static String aw(Context context, int i) {
