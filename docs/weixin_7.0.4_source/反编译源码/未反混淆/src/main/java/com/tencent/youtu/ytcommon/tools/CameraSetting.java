...
public class CameraSetting {
    ...
    public static int getRotateTag(int i) {
        ...
        if (i == 90) {
            ...
            YTLogger.w(TAG, "获取摄像头转向结果的时候值不为90或者180");
        }
    ...
    public static int getVideoRotate(Context context, int i) {
        ...
        YTLogger.d(TAG, "debug camera orientation is " + cameraInfo.orientation + " ui degrees is " + i2);
    ...
    public static int initCamera(Context context, Camera camera, int i) {
        ...
        try {
            ...
            for (i2 = 0; i2 < supportedFocusModes.size(); i2++) {
                YTLogger.v(TAG, "suporrtedFocusModes " + i2 + " :" + ((String) supportedFocusModes.get(i2)));
            }
            ...
            try {
                ...
                YTLogger.w(TAG, "Camera.setParameters.setPreviewSize failed!!: " + e.getLocalizedMessage());
            ...
            YTLogger.d(TAG, "videoOrietation is".concat(String.valueOf(i2)));
            if (CamcorderProfile.hasProfile(i, 4)) {
                camcorderProfile = CamcorderProfile.get(i, 4);
                YTLogger.d(TAG, "480P camcorderProfile:" + camcorderProfile.videoFrameWidth + VideoMaterialUtil.CRAZYFACE_X + camcorderProfile.videoFrameHeight);
            } else if (CamcorderProfile.hasProfile(i, 5)) {
                camcorderProfile = CamcorderProfile.get(i, 5);
                YTLogger.d(TAG, "720P camcorderProfile:" + camcorderProfile.videoFrameWidth + VideoMaterialUtil.CRAZYFACE_X + camcorderProfile.videoFrameHeight);
            } else {
                camcorderProfile = CamcorderProfile.get(i, 1);
                YTLogger.d(TAG, "High camcorderProfile:" + camcorderProfile.videoFrameWidth + VideoMaterialUtil.CRAZYFACE_X + camcorderProfile.videoFrameHeight);
            }
            ...
            parameters = camera.getParameters();
            YTLogger.d(TAG, "choose camera fps is : ".concat(String.valueOf(chooseFixedPreviewFps(parameters, 30000))));
            try {
                camera.setParameters(parameters);
            } catch (Exception e22) {
                YTLogger.w(TAG, "Camera.setParameters.preview fps failed!!: " + e22.getLocalizedMessage());
                YTException.report(e22);
            }
            parameters = camera.getParameters();
            int[] iArr = new int[2];
            parameters.getPreviewFpsRange(iArr);
            YTLogger.d(TAG, "after set parameters getPreviewFpsRange=" + iArr[0] + "-" + iArr[1] + " ;after set parameter fps=" + parameters.getPreviewFrameRate());
    ...
    public static void setVideoSize(Parameters parameters, CamcorderProfile camcorderProfile) {
        ...
        YTLogger.d(TAG, "select video size camcorderProfile:" + camcorderProfile.videoFrameWidth + VideoMaterialUtil.CRAZYFACE_X + camcorderProfile.videoFrameHeight);
