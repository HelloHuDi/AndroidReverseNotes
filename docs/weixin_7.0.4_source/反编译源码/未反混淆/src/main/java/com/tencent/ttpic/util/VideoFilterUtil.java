...
public class VideoFilterUtil {
        ...
        if (videoMaterial.getFabbyParts() != null) {
            ...
            for (FabbyMvPart fabbyMvPart : videoMaterial.getFabbyParts().getParts()) {
                ...
                if (fabbyMvPart.filterType == 1) {
                    ...
                    fabbyMvFilter.mEffectFilter = new ab();
                    fabbyMvFilter.mEffectFilter.addParam(new k("inputImageTexture2", BitmapUtils.decodeSampledBitmapFromFile(videoMaterial.getDataPath() + File.separator + fabbyMvPart.lutFile, 1), 33986, true));
                }
    ...
    public static h getSecondLastFrame(h hVar) {
        ...
        if (FrameUtil.isValid(hVar) && FrameUtil.isValid(hVar.bJA)) {
            while (FrameUtil.isValid(hVar.bJA) && FrameUtil.isValid(hVar.bJA.bJA)) {
                hVar = hVar.bJA;
            }
        ...
        if (itemList != null) {
            ...
            while (true) {
                ...
                if (stickerItem.particleConfig != null && VideoMaterialUtil.isFaceItem(stickerItem)) {
                    arrayList.add(new ParticleFilter(videoMaterial.getDataPath() + File.separator + stickerItem.id, stickerItem));
                }
                i = i2 + 1;
            }
        }
