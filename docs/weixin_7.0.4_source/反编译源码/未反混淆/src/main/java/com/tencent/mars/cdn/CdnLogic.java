...
public class CdnLogic {
    ...
    public static void UnInitialize() {
        ab.w(TAG, "uninit cdnlogic");
        setAppCallback(null);
    }

    public static int doCertificateVerify(String str, byte[][] bArr) {
        int i = -1;
        ab.i(TAG, "certifivate verify for %s", str);
        try {
            ab.i(TAG, "host %s verify result %d, isknownroots %b", str, Integer.valueOf(r1.getStatus()), Boolean.valueOf(X509Util.verifyServerCertificates(bArr, "RSA", str).isIssuedByKnownRoot()));
            return X509Util.verifyServerCertificates(bArr, "RSA", str).getStatus();
        } catch (KeyStoreException e) {
            ab.e(TAG, e.getLocalizedMessage());
            return i;
        } catch (NoSuchAlgorithmException e2) {
            ab.e(TAG, e2.getLocalizedMessage());
            return i;
        } catch (IllegalArgumentException e3) {
            ab.e(TAG, e3.getLocalizedMessage());
            return i;
        } catch (Exception e4) {
            ab.e(TAG, e4.getLocalizedMessage());
            return i;
        }
    }
}
