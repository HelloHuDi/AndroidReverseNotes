package com.tencent.mm.protocal.protobuf;

import com.tencent.matrix.trace.core.AppMethodBeat;
import com.tencent.mm.bt.a;
import e.a.a.b;
import java.util.LinkedList;

public final class gh extends a {
    public SKBuiltinBuffer_t vIq;
    public xu vIr;

    public final int op(int i, Object... objArr) {
        AppMethodBeat.i(58890);
        b bVar;
        int ix;
        if (i == 0) {
            e.a.a.c.a aVar = (e.a.a.c.a) objArr[0];
            if (this.vIq == null) {
                bVar = new b("Not all required fields were included: AesEncryptKey");
                AppMethodBeat.o(58890);
                throw bVar;
            }
            if (this.vIq != null) {
                aVar.iy(2, this.vIq.computeSize());
                this.vIq.writeFields(aVar);
            }
            if (this.vIr != null) {
                aVar.iy(3, this.vIr.computeSize());
                this.vIr.writeFields(aVar);
            }
            AppMethodBeat.o(58890);
            return 0;
        } else if (i == 1) {
            if (this.vIq != null) {
                ix = e.a.a.a.ix(2, this.vIq.computeSize()) + 0;
            } else {
                ix = 0;
            }
            if (this.vIr != null) {
                ix += e.a.a.a.ix(3, this.vIr.computeSize());
            }
            AppMethodBeat.o(58890);
            return ix;
        } else if (i == 2) {
            e.a.a.a.a aVar2 = new e.a.a.a.a((byte[]) objArr[0], unknownTagHandler);
            for (ix = a.getNextFieldNumber(aVar2); ix > 0; ix = a.getNextFieldNumber(aVar2)) {
                if (!super.populateBuilderWithField(aVar2, this, ix)) {
                    aVar2.ems();
                }
            }
            if (this.vIq == null) {
                bVar = new b("Not all required fields were included: AesEncryptKey");
                AppMethodBeat.o(58890);
                throw bVar;
            }
            AppMethodBeat.o(58890);
            return 0;
        } else if (i == 3) {
            e.a.a.a.a aVar3 = (e.a.a.a.a) objArr[0];
            gh ghVar = (gh) objArr[1];
            int intValue = ((Integer) objArr[2]).intValue();
            LinkedList Vh;
            int size;
            byte[] bArr;
            e.a.a.a.a aVar4;
            boolean z;
            switch (intValue) {
                case 2:
                    Vh = aVar3.Vh(intValue);
                    size = Vh.size();
                    for (intValue = 0; intValue < size; intValue++) {
                        bArr = (byte[]) Vh.get(intValue);
                        SKBuiltinBuffer_t sKBuiltinBuffer_t = new SKBuiltinBuffer_t();
                        aVar4 = new e.a.a.a.a(bArr, unknownTagHandler);
                        for (z = true; z; z = sKBuiltinBuffer_t.populateBuilderWithField(aVar4, sKBuiltinBuffer_t, a.getNextFieldNumber(aVar4))) {
                        }
                        ghVar.vIq = sKBuiltinBuffer_t;
                    }
                    AppMethodBeat.o(58890);
                    return 0;
                case 3:
                    Vh = aVar3.Vh(intValue);
                    size = Vh.size();
                    for (intValue = 0; intValue < size; intValue++) {
                        bArr = (byte[]) Vh.get(intValue);
                        xu xuVar = new xu();
                        aVar4 = new e.a.a.a.a(bArr, unknownTagHandler);
                        for (z = true; z; z = xuVar.populateBuilderWithField(aVar4, xuVar, a.getNextFieldNumber(aVar4))) {
                        }
                        ghVar.vIr = xuVar;
                    }
                    AppMethodBeat.o(58890);
                    return 0;
                default:
                    AppMethodBeat.o(58890);
                    return -1;
            }
        } else {
            AppMethodBeat.o(58890);
            return -1;
        }
    }
}
