package com.tencent.mm.protocal.protobuf;

import com.tencent.matrix.trace.core.AppMethodBeat;
import e.a.a.b;
import e.a.a.c.a;
import java.util.LinkedList;

public final class beb extends bsr {
    public String jBB;
    public String luQ;
    public SKBuiltinBuffer_t wHH;
    public SKBuiltinBuffer_t wHI;

    public final int op(int i, Object... objArr) {
        AppMethodBeat.i(58917);
        b bVar;
        int ix;
        if (i == 0) {
            a aVar = (a) objArr[0];
            if (this.wHH == null) {
                bVar = new b("Not all required fields were included: CurrentSynckey");
                AppMethodBeat.o(58917);
                throw bVar;
            } else if (this.wHI == null) {
                bVar = new b("Not all required fields were included: MaxSynckey");
                AppMethodBeat.o(58917);
                throw bVar;
            } else {
                if (this.BaseRequest != null) {
                    aVar.iy(1, this.BaseRequest.computeSize());
                    this.BaseRequest.writeFields(aVar);
                }
                if (this.jBB != null) {
                    aVar.e(2, this.jBB);
                }
                if (this.wHH != null) {
                    aVar.iy(3, this.wHH.computeSize());
                    this.wHH.writeFields(aVar);
                }
                if (this.wHI != null) {
                    aVar.iy(4, this.wHI.computeSize());
                    this.wHI.writeFields(aVar);
                }
                if (this.luQ != null) {
                    aVar.e(5, this.luQ);
                }
                AppMethodBeat.o(58917);
                return 0;
            }
        } else if (i == 1) {
            if (this.BaseRequest != null) {
                ix = e.a.a.a.ix(1, this.BaseRequest.computeSize()) + 0;
            } else {
                ix = 0;
            }
            if (this.jBB != null) {
                ix += e.a.a.b.b.a.f(2, this.jBB);
            }
            if (this.wHH != null) {
                ix += e.a.a.a.ix(3, this.wHH.computeSize());
            }
            if (this.wHI != null) {
                ix += e.a.a.a.ix(4, this.wHI.computeSize());
            }
            if (this.luQ != null) {
                ix += e.a.a.b.b.a.f(5, this.luQ);
            }
            AppMethodBeat.o(58917);
            return ix;
        } else if (i == 2) {
            e.a.a.a.a aVar2 = new e.a.a.a.a((byte[]) objArr[0], unknownTagHandler);
            for (ix = com.tencent.mm.bt.a.getNextFieldNumber(aVar2); ix > 0; ix = com.tencent.mm.bt.a.getNextFieldNumber(aVar2)) {
                if (!super.populateBuilderWithField(aVar2, this, ix)) {
                    aVar2.ems();
                }
            }
            if (this.wHH == null) {
                bVar = new b("Not all required fields were included: CurrentSynckey");
                AppMethodBeat.o(58917);
                throw bVar;
            } else if (this.wHI == null) {
                bVar = new b("Not all required fields were included: MaxSynckey");
                AppMethodBeat.o(58917);
                throw bVar;
            } else {
                AppMethodBeat.o(58917);
                return 0;
            }
        } else if (i == 3) {
            e.a.a.a.a aVar3 = (e.a.a.a.a) objArr[0];
            beb beb = (beb) objArr[1];
            int intValue = ((Integer) objArr[2]).intValue();
            LinkedList Vh;
            int size;
            byte[] bArr;
            e.a.a.a.a aVar4;
            boolean z;
            SKBuiltinBuffer_t sKBuiltinBuffer_t;
            switch (intValue) {
                case 1:
                    Vh = aVar3.Vh(intValue);
                    size = Vh.size();
                    for (intValue = 0; intValue < size; intValue++) {
                        bArr = (byte[]) Vh.get(intValue);
                        hl hlVar = new hl();
                        aVar4 = new e.a.a.a.a(bArr, unknownTagHandler);
                        for (z = true; z; z = hlVar.populateBuilderWithField(aVar4, hlVar, com.tencent.mm.bt.a.getNextFieldNumber(aVar4))) {
                        }
                        beb.BaseRequest = hlVar;
                    }
                    AppMethodBeat.o(58917);
                    return 0;
                case 2:
                    beb.jBB = aVar3.BTU.readString();
                    AppMethodBeat.o(58917);
                    return 0;
                case 3:
                    Vh = aVar3.Vh(intValue);
                    size = Vh.size();
                    for (intValue = 0; intValue < size; intValue++) {
                        bArr = (byte[]) Vh.get(intValue);
                        sKBuiltinBuffer_t = new SKBuiltinBuffer_t();
                        aVar4 = new e.a.a.a.a(bArr, unknownTagHandler);
                        for (z = true; z; z = sKBuiltinBuffer_t.populateBuilderWithField(aVar4, sKBuiltinBuffer_t, com.tencent.mm.bt.a.getNextFieldNumber(aVar4))) {
                        }
                        beb.wHH = sKBuiltinBuffer_t;
                    }
                    AppMethodBeat.o(58917);
                    return 0;
                case 4:
                    Vh = aVar3.Vh(intValue);
                    size = Vh.size();
                    for (intValue = 0; intValue < size; intValue++) {
                        bArr = (byte[]) Vh.get(intValue);
                        sKBuiltinBuffer_t = new SKBuiltinBuffer_t();
                        aVar4 = new e.a.a.a.a(bArr, unknownTagHandler);
                        for (z = true; z; z = sKBuiltinBuffer_t.populateBuilderWithField(aVar4, sKBuiltinBuffer_t, com.tencent.mm.bt.a.getNextFieldNumber(aVar4))) {
                        }
                        beb.wHI = sKBuiltinBuffer_t;
                    }
                    AppMethodBeat.o(58917);
                    return 0;
                case 5:
                    beb.luQ = aVar3.BTU.readString();
                    AppMethodBeat.o(58917);
                    return 0;
                default:
                    AppMethodBeat.o(58917);
                    return -1;
            }
        } else {
            AppMethodBeat.o(58917);
            return -1;
        }
    }
}
