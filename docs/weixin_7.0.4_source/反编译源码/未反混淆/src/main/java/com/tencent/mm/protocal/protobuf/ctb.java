package com.tencent.mm.protocal.protobuf;

import com.tencent.matrix.trace.core.AppMethodBeat;
import e.a.a.c.a;
import java.util.LinkedList;

public final class ctb extends bsr {
    public String csB;
    public int id;
    public String query;
    public LinkedList<Integer> xqi = new LinkedList();

    public ctb() {
        AppMethodBeat.i(93816);
        AppMethodBeat.o(93816);
    }

    public final int op(int i, Object... objArr) {
        AppMethodBeat.i(93817);
        int ix;
        byte[] bArr;
        if (i == 0) {
            a aVar = (a) objArr[0];
            if (this.BaseRequest != null) {
                aVar.iy(1, this.BaseRequest.computeSize());
                this.BaseRequest.writeFields(aVar);
            }
            if (this.csB != null) {
                aVar.e(2, this.csB);
            }
            aVar.iz(3, this.id);
            aVar.e(4, 2, this.xqi);
            if (this.query != null) {
                aVar.e(5, this.query);
            }
            AppMethodBeat.o(93817);
            return 0;
        } else if (i == 1) {
            if (this.BaseRequest != null) {
                ix = e.a.a.a.ix(1, this.BaseRequest.computeSize()) + 0;
            } else {
                ix = 0;
            }
            if (this.csB != null) {
                ix += e.a.a.b.b.a.f(2, this.csB);
            }
            ix = (ix + e.a.a.b.b.a.bs(3, this.id)) + e.a.a.a.c(4, 2, this.xqi);
            if (this.query != null) {
                ix += e.a.a.b.b.a.f(5, this.query);
            }
            AppMethodBeat.o(93817);
            return ix;
        } else if (i == 2) {
            bArr = (byte[]) objArr[0];
            this.xqi.clear();
            e.a.a.a.a aVar2 = new e.a.a.a.a(bArr, unknownTagHandler);
            for (ix = com.tencent.mm.bt.a.getNextFieldNumber(aVar2); ix > 0; ix = com.tencent.mm.bt.a.getNextFieldNumber(aVar2)) {
                if (!super.populateBuilderWithField(aVar2, this, ix)) {
                    aVar2.ems();
                }
            }
            AppMethodBeat.o(93817);
            return 0;
        } else if (i == 3) {
            e.a.a.a.a aVar3 = (e.a.a.a.a) objArr[0];
            ctb ctb = (ctb) objArr[1];
            int intValue = ((Integer) objArr[2]).intValue();
            switch (intValue) {
                case 1:
                    LinkedList Vh = aVar3.Vh(intValue);
                    int size = Vh.size();
                    for (intValue = 0; intValue < size; intValue++) {
                        bArr = (byte[]) Vh.get(intValue);
                        hl hlVar = new hl();
                        e.a.a.a.a aVar4 = new e.a.a.a.a(bArr, unknownTagHandler);
                        for (boolean z = true; z; z = hlVar.populateBuilderWithField(aVar4, hlVar, com.tencent.mm.bt.a.getNextFieldNumber(aVar4))) {
                        }
                        ctb.BaseRequest = hlVar;
                    }
                    AppMethodBeat.o(93817);
                    return 0;
                case 2:
                    ctb.csB = aVar3.BTU.readString();
                    AppMethodBeat.o(93817);
                    return 0;
                case 3:
                    ctb.id = aVar3.BTU.vd();
                    AppMethodBeat.o(93817);
                    return 0;
                case 4:
                    ctb.xqi.add(Integer.valueOf(aVar3.BTU.vd()));
                    AppMethodBeat.o(93817);
                    return 0;
                case 5:
                    ctb.query = aVar3.BTU.readString();
                    AppMethodBeat.o(93817);
                    return 0;
                default:
                    AppMethodBeat.o(93817);
                    return -1;
            }
        } else {
            AppMethodBeat.o(93817);
            return -1;
        }
    }
}
