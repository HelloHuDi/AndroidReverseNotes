package com.tencent.mm.protocal.protobuf;

import com.tencent.matrix.trace.core.AppMethodBeat;
import e.a.a.c.a;
import java.util.LinkedList;

public final class cwl extends bsr {
    public String csB;
    public LinkedList<String> xse = new LinkedList();
    public boolean xsf;
    public String xsg;

    public cwl() {
        AppMethodBeat.i(114997);
        AppMethodBeat.o(114997);
    }

    public final int op(int i, Object... objArr) {
        AppMethodBeat.i(114998);
        int ix;
        byte[] bArr;
        if (i == 0) {
            a aVar = (a) objArr[0];
            if (this.BaseRequest != null) {
                aVar.iy(1, this.BaseRequest.computeSize());
                this.BaseRequest.writeFields(aVar);
            }
            if (this.csB != null) {
                aVar.e(2, this.csB);
            }
            aVar.e(3, 1, this.xse);
            aVar.aO(4, this.xsf);
            if (this.xsg != null) {
                aVar.e(5, this.xsg);
            }
            AppMethodBeat.o(114998);
            return 0;
        } else if (i == 1) {
            if (this.BaseRequest != null) {
                ix = e.a.a.a.ix(1, this.BaseRequest.computeSize()) + 0;
            } else {
                ix = 0;
            }
            if (this.csB != null) {
                ix += e.a.a.b.b.a.f(2, this.csB);
            }
            ix = (ix + e.a.a.a.c(3, 1, this.xse)) + (e.a.a.b.b.a.fv(4) + 1);
            if (this.xsg != null) {
                ix += e.a.a.b.b.a.f(5, this.xsg);
            }
            AppMethodBeat.o(114998);
            return ix;
        } else if (i == 2) {
            bArr = (byte[]) objArr[0];
            this.xse.clear();
            e.a.a.a.a aVar2 = new e.a.a.a.a(bArr, unknownTagHandler);
            for (ix = com.tencent.mm.bt.a.getNextFieldNumber(aVar2); ix > 0; ix = com.tencent.mm.bt.a.getNextFieldNumber(aVar2)) {
                if (!super.populateBuilderWithField(aVar2, this, ix)) {
                    aVar2.ems();
                }
            }
            AppMethodBeat.o(114998);
            return 0;
        } else if (i == 3) {
            e.a.a.a.a aVar3 = (e.a.a.a.a) objArr[0];
            cwl cwl = (cwl) objArr[1];
            int intValue = ((Integer) objArr[2]).intValue();
            switch (intValue) {
                case 1:
                    LinkedList Vh = aVar3.Vh(intValue);
                    int size = Vh.size();
                    for (intValue = 0; intValue < size; intValue++) {
                        bArr = (byte[]) Vh.get(intValue);
                        hl hlVar = new hl();
                        e.a.a.a.a aVar4 = new e.a.a.a.a(bArr, unknownTagHandler);
                        for (boolean z = true; z; z = hlVar.populateBuilderWithField(aVar4, hlVar, com.tencent.mm.bt.a.getNextFieldNumber(aVar4))) {
                        }
                        cwl.BaseRequest = hlVar;
                    }
                    AppMethodBeat.o(114998);
                    return 0;
                case 2:
                    cwl.csB = aVar3.BTU.readString();
                    AppMethodBeat.o(114998);
                    return 0;
                case 3:
                    cwl.xse.add(aVar3.BTU.readString());
                    AppMethodBeat.o(114998);
                    return 0;
                case 4:
                    cwl.xsf = aVar3.BTU.ehX();
                    AppMethodBeat.o(114998);
                    return 0;
                case 5:
                    cwl.xsg = aVar3.BTU.readString();
                    AppMethodBeat.o(114998);
                    return 0;
                default:
                    AppMethodBeat.o(114998);
                    return -1;
            }
        } else {
            AppMethodBeat.o(114998);
            return -1;
        }
    }
}
