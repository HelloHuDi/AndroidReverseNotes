package com.tencent.mm.model;

import com.tencent.matrix.trace.core.AppMethodBeat;
import com.tencent.mm.bt.a;
import java.util.LinkedList;

public final class bu extends a {
    public LinkedList<bt> fnt = new LinkedList();

    public bu() {
        AppMethodBeat.i(72615);
        AppMethodBeat.o(72615);
    }

    public final int op(int i, Object... objArr) {
        AppMethodBeat.i(72616);
        int c;
        byte[] bArr;
        if (i == 0) {
            ((e.a.a.c.a) objArr[0]).e(1, 8, this.fnt);
            AppMethodBeat.o(72616);
            return 0;
        } else if (i == 1) {
            c = e.a.a.a.c(1, 8, this.fnt) + 0;
            AppMethodBeat.o(72616);
            return c;
        } else if (i == 2) {
            bArr = (byte[]) objArr[0];
            this.fnt.clear();
            e.a.a.a.a aVar = new e.a.a.a.a(bArr, unknownTagHandler);
            for (c = a.getNextFieldNumber(aVar); c > 0; c = a.getNextFieldNumber(aVar)) {
                if (!super.populateBuilderWithField(aVar, this, c)) {
                    aVar.ems();
                }
            }
            AppMethodBeat.o(72616);
            return 0;
        } else if (i == 3) {
            e.a.a.a.a aVar2 = (e.a.a.a.a) objArr[0];
            bu buVar = (bu) objArr[1];
            int intValue = ((Integer) objArr[2]).intValue();
            switch (intValue) {
                case 1:
                    LinkedList Vh = aVar2.Vh(intValue);
                    int size = Vh.size();
                    for (intValue = 0; intValue < size; intValue++) {
                        bArr = (byte[]) Vh.get(intValue);
                        bt btVar = new bt();
                        e.a.a.a.a aVar3 = new e.a.a.a.a(bArr, unknownTagHandler);
                        for (boolean z = true; z; z = btVar.populateBuilderWithField(aVar3, btVar, a.getNextFieldNumber(aVar3))) {
                        }
                        buVar.fnt.add(btVar);
                    }
                    AppMethodBeat.o(72616);
                    return 0;
                default:
                    AppMethodBeat.o(72616);
                    return -1;
            }
        } else {
            AppMethodBeat.o(72616);
            return -1;
        }
    }
}
