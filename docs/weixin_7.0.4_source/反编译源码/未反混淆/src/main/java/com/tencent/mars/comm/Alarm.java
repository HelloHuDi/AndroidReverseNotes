...
public class Alarm extends BroadcastReceiver {
    ...
    public static boolean start(long j, int i, Context context) {
        ...
        if (i < 0) {
            ab.e(TAG, "id:%d, after:%d", Long.valueOf(j), Integer.valueOf(i));
            return false;
        } else if (context == null) {
            ab.e(TAG, "null==context, id:%d, after:%d", Long.valueOf(j), Integer.valueOf(i));
            return false;
        } else {
            synchronized (gPendingAlarms) {
                if (wakerlock == null) {
                    wakerlock = new WakerLock(context, TAG);
                    ab.i(TAG, "start new wakerlock");
                }
                if (bc_alarm == null) {
                    bc_alarm = new Alarm();
                ...
                if (gPendingAlarms.containsKey(Long.valueOf(j))) {
                    ab.e(TAG, "id exist=%d", Long.valueOf(j));
                    return false;
                }
                if (i >= 0) {
                ...
                ab.i(TAG, "Alarm.start [id: %d, after: %d, size: %d]", Long.valueOf(j), Integer.valueOf(i), Integer.valueOf(gPendingAlarms.size()));
                return true;
            }
        }
    }
    ...
    public static boolean stop(long j, Context context) {
        ab.i(TAG, "Alarm.stop [id: %d]", Long.valueOf(j));
        if (context == null) {
            ab.e(TAG, "context==null");
            return false;
        }
        synchronized (gPendingAlarms) {
            if (wakerlock == null) {
                wakerlock = new WakerLock(context, TAG);
                ab.i(TAG, "stop new wakerlock");
            }
            if (bc_alarm == null) {
                bc_alarm = new Alarm();
                context.registerReceiver(bc_alarm, new IntentFilter());
                ab.i(TAG, "stop new Alarm");
            }
        ...
        if (context != null && intent != null) {
            ...
            if (0 != longExtra && intExtra != 0) {
                if (intExtra != Process.myPid()) {
                    ab.w(TAG, "onReceive id:%d, pid:%d, mypid:%d", Long.valueOf(longExtra), Integer.valueOf(intExtra), Integer.valueOf(Process.myPid()));
                    return;
                }
                synchronized (gPendingAlarms) {
                    if (((AlarmRecord) gPendingAlarms.remove(Long.valueOf(longExtra))) != null) {
                        ab.i(TAG, "Alarm.onReceive [id: %d, delta miss time: %d, size: %d]", Long.valueOf(longExtra), Long.valueOf(SystemClock.elapsedRealtime() - ((AlarmRecord) gPendingAlarms.remove(Long.valueOf(longExtra))).waitTime), Integer.valueOf(gPendingAlarms.size()));
                        if (wakerlock != null) {
                            wakerlock.lock(200, "Alarm.onReceive");
                        }
                        onAlarm(longExtra);
                    } else {
                        ab.e(TAG, "onReceive not found id:%d, pid:%d, gPendingAlarms.size:%d", Long.valueOf(longExtra), Integer.valueOf(intExtra), Integer.valueOf(gPendingAlarms.size()));
                    }
                }
            }
        }
