    ...
    public static void adjustCoords(PointF[] pointFArr, PointF[] pointFArr2, List<FaceMoveItem> list) {
        ...
        while (true) {
            ...
            if (i2 < list.size()) {
                ...
                pointFArr2[faceMoveItem.position] = new PointF((pointF2.x + (pointF3.x * ((float) faceMoveItem.dx))) + (pointF4.x * ((float) faceMoveItem.dy)), (((float) faceMoveItem.dy) * pointF4.y) + (pointF2.y + (pointF3.y * ((float) faceMoveItem.dx))));
                i = i2 + 1;
            } else {
                AppMethodBeat.o(83882);
                return;
