        ...
        if (0 == valueOf.longValue() || valueOf2.intValue() == 0) {
            ...
            synchronized (alarm_waiting_set) {
                try {
                    Iterator it = alarm_waiting_set.iterator();
                    while (it.hasNext()) {
                        Log.i(TAG, "onReceive id=%d, curId=%d", valueOf, (Long) ((Object[]) it.next())[TSetData.ID.ordinal()]);
                        if (((Long) ((Object[]) it.next())[TSetData.ID.ordinal()]).equals(valueOf)) {
                            Log.i(TAG, "onReceive find alarm id:%d, pid:%d, delta miss time:%d", valueOf, valueOf2, Long.valueOf(SystemClock.elapsedRealtime() - ((Long) r2[TSetData.WAITTIME.ordinal()]).longValue()));
                            it.remove();
                            obj = 1;
                            break;
                        }
