...
public final class CursorToBulkCursorAdaptor extends BulkCursorNative implements DeathRecipient {
    ...
    private void throwIfCursorIsClosed() {
        AppMethodBeat.i(TXCStreamDownloader.TXE_DOWNLOAD_INFO_SERVER_REFUSE);
        if (this.mCursor == null) {
            StaleDataException staleDataException = new StaleDataException("Attempted to access a cursor after it has been closed.");
            AppMethodBeat.o(TXCStreamDownloader.TXE_DOWNLOAD_INFO_SERVER_REFUSE);
    ...
    public final BulkCursorDescriptor getBulkCursorDescriptor() {
        ...
        AppMethodBeat.i(TXCStreamDownloader.TXE_DOWNLOAD_ERROR_CONNECT_FAILED);
        synchronized (this.mLock) {
            try {
                ...
                AppMethodBeat.o(TXCStreamDownloader.TXE_DOWNLOAD_ERROR_CONNECT_FAILED);
            }
        }
    ...
    public final CursorWindow getWindow(int i) {
        CursorWindow window;
        AppMethodBeat.i(TXCStreamDownloader.TXE_DOWNLOAD_ERROR_DISCONNECT);
        synchronized (this.mLock) {
            try {
                throwIfCursorIsClosed();
                if (this.mCursor.moveToPosition(i)) {
                    ...
                    AppMethodBeat.o(TXCStreamDownloader.TXE_DOWNLOAD_ERROR_DISCONNECT);
                } else {
                    closeFilledWindowLocked();
                    window = null;
                }
    ...
    public final void onMove(int i) {
        AppMethodBeat.i(TXCStreamDownloader.TXE_DOWNLOAD_ERROR_READ_FAILED);
        synchronized (this.mLock) {
            try {
                throwIfCursorIsClosed();
                ...
                AppMethodBeat.o(TXCStreamDownloader.TXE_DOWNLOAD_ERROR_READ_FAILED);
            }
        }
    }

    public final void deactivate() {
        AppMethodBeat.i(TXCStreamDownloader.TXE_DOWNLOAD_ERROR_WRITE_FAILED);
        synchronized (this.mLock) {
            try {
                if (this.mCursor != null) {
                    unregisterObserverProxyLocked();
                ...
                AppMethodBeat.o(TXCStreamDownloader.TXE_DOWNLOAD_ERROR_WRITE_FAILED);
            }
        }
    }

    public final void close() {
        AppMethodBeat.i(TXCStreamDownloader.TXE_DOWNLOAD_ERROR_NET_RECONNECT);
        synchronized (this.mLock) {
            try {
                disposeLocked();
            } finally {
                ...
                AppMethodBeat.o(TXCStreamDownloader.TXE_DOWNLOAD_ERROR_NET_RECONNECT);
            }
        }
    }
