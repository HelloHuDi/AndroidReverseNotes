        ...
        try {
            ...
            try {
                ...
                if (readUnsignedInt != WAV_HEADER_CHUNK_ID) {
                    Logger.i(TAG, "isWavFormat Invalid WAVE header chunk ID = ".concat(String.valueOf(readUnsignedInt)));
                    try {
                ...
                if (readUnsignedInt != WAV_FORMAT) {
                    Logger.i(TAG, "isWavFormat Invalid WAVE format = ".concat(String.valueOf(readUnsignedInt)));
                    try {
                        fileInputStream.close();
                    } catch (Exception e32) {
                        Logger.e(TAG, e32);
                ...
                if (readUnsignedInt != WAV_FORMAT_CHUNK_ID) {
                    Logger.i(TAG, "isWavFormat Invalid WAVE format chunk ID formatId = ".concat(String.valueOf(readUnsignedInt)));
                    try {
                        fileInputStream.close();
                ...
                Logger.i(TAG, "isWavFormat formatSize = ".concat(String.valueOf(readUnsignedIntLE(bArr, 16))));
                Logger.i(TAG, "isWavFormat mAudioFormat = ".concat(String.valueOf(readUnsignedShortLE(bArr, 20))));
                try {
                    fileInputStream.close();
                } catch (Exception e22) {
                    Logger.e(TAG, e22);
