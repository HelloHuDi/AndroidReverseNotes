package com.tencent.mm.protocal.protobuf;

import com.tencent.matrix.trace.core.AppMethodBeat;
import e.a.a.b;
import e.a.a.c.a;
import java.util.LinkedList;

public final class bnn extends btd {
    public int kdT;
    public String kdU;
    public String wNr;
    public String wNs;
    public mu wpq;
    public atk wps;

    public final int op(int i, Object... objArr) {
        AppMethodBeat.i(48938);
        b bVar;
        int ix;
        if (i == 0) {
            a aVar = (a) objArr[0];
            if (this.BaseResponse == null) {
                bVar = new b("Not all required fields were included: BaseResponse");
                AppMethodBeat.o(48938);
                throw bVar;
            }
            if (this.BaseResponse != null) {
                aVar.iy(1, this.BaseResponse.computeSize());
                this.BaseResponse.writeFields(aVar);
            }
            aVar.iz(2, this.kdT);
            if (this.kdU != null) {
                aVar.e(3, this.kdU);
            }
            if (this.wps != null) {
                aVar.iy(4, this.wps.computeSize());
                this.wps.writeFields(aVar);
            }
            if (this.wNr != null) {
                aVar.e(5, this.wNr);
            }
            if (this.wNs != null) {
                aVar.e(6, this.wNs);
            }
            if (this.wpq != null) {
                aVar.iy(7, this.wpq.computeSize());
                this.wpq.writeFields(aVar);
            }
            AppMethodBeat.o(48938);
            return 0;
        } else if (i == 1) {
            if (this.BaseResponse != null) {
                ix = e.a.a.a.ix(1, this.BaseResponse.computeSize()) + 0;
            } else {
                ix = 0;
            }
            ix += e.a.a.b.b.a.bs(2, this.kdT);
            if (this.kdU != null) {
                ix += e.a.a.b.b.a.f(3, this.kdU);
            }
            if (this.wps != null) {
                ix += e.a.a.a.ix(4, this.wps.computeSize());
            }
            if (this.wNr != null) {
                ix += e.a.a.b.b.a.f(5, this.wNr);
            }
            if (this.wNs != null) {
                ix += e.a.a.b.b.a.f(6, this.wNs);
            }
            if (this.wpq != null) {
                ix += e.a.a.a.ix(7, this.wpq.computeSize());
            }
            AppMethodBeat.o(48938);
            return ix;
        } else if (i == 2) {
            e.a.a.a.a aVar2 = new e.a.a.a.a((byte[]) objArr[0], unknownTagHandler);
            for (ix = com.tencent.mm.bt.a.getNextFieldNumber(aVar2); ix > 0; ix = com.tencent.mm.bt.a.getNextFieldNumber(aVar2)) {
                if (!super.populateBuilderWithField(aVar2, this, ix)) {
                    aVar2.ems();
                }
            }
            if (this.BaseResponse == null) {
                bVar = new b("Not all required fields were included: BaseResponse");
                AppMethodBeat.o(48938);
                throw bVar;
            }
            AppMethodBeat.o(48938);
            return 0;
        } else if (i == 3) {
            e.a.a.a.a aVar3 = (e.a.a.a.a) objArr[0];
            bnn bnn = (bnn) objArr[1];
            int intValue = ((Integer) objArr[2]).intValue();
            LinkedList Vh;
            int size;
            byte[] bArr;
            e.a.a.a.a aVar4;
            boolean z;
            switch (intValue) {
                case 1:
                    Vh = aVar3.Vh(intValue);
                    size = Vh.size();
                    for (intValue = 0; intValue < size; intValue++) {
                        bArr = (byte[]) Vh.get(intValue);
                        BaseResponse baseResponse = new BaseResponse();
                        aVar4 = new e.a.a.a.a(bArr, unknownTagHandler);
                        for (z = true; z; z = baseResponse.populateBuilderWithField(aVar4, baseResponse, com.tencent.mm.bt.a.getNextFieldNumber(aVar4))) {
                        }
                        bnn.BaseResponse = baseResponse;
                    }
                    AppMethodBeat.o(48938);
                    return 0;
                case 2:
                    bnn.kdT = aVar3.BTU.vd();
                    AppMethodBeat.o(48938);
                    return 0;
                case 3:
                    bnn.kdU = aVar3.BTU.readString();
                    AppMethodBeat.o(48938);
                    return 0;
                case 4:
                    Vh = aVar3.Vh(intValue);
                    size = Vh.size();
                    for (intValue = 0; intValue < size; intValue++) {
                        bArr = (byte[]) Vh.get(intValue);
                        atk atk = new atk();
                        aVar4 = new e.a.a.a.a(bArr, unknownTagHandler);
                        for (z = true; z; z = atk.populateBuilderWithField(aVar4, atk, com.tencent.mm.bt.a.getNextFieldNumber(aVar4))) {
                        }
                        bnn.wps = atk;
                    }
                    AppMethodBeat.o(48938);
                    return 0;
                case 5:
                    bnn.wNr = aVar3.BTU.readString();
                    AppMethodBeat.o(48938);
                    return 0;
                case 6:
                    bnn.wNs = aVar3.BTU.readString();
                    AppMethodBeat.o(48938);
                    return 0;
                case 7:
                    Vh = aVar3.Vh(intValue);
                    size = Vh.size();
                    for (intValue = 0; intValue < size; intValue++) {
                        bArr = (byte[]) Vh.get(intValue);
                        mu muVar = new mu();
                        aVar4 = new e.a.a.a.a(bArr, unknownTagHandler);
                        for (z = true; z; z = muVar.populateBuilderWithField(aVar4, muVar, com.tencent.mm.bt.a.getNextFieldNumber(aVar4))) {
                        }
                        bnn.wpq = muVar;
                    }
                    AppMethodBeat.o(48938);
                    return 0;
                default:
                    AppMethodBeat.o(48938);
                    return -1;
            }
        } else {
            AppMethodBeat.o(48938);
            return -1;
        }
    }
}
