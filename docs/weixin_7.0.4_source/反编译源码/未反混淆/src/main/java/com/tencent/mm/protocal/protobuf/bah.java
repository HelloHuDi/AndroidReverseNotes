package com.tencent.mm.protocal.protobuf;

import com.tencent.matrix.trace.core.AppMethodBeat;
import com.tencent.mm.bt.a;
import e.a.a.b;
import java.util.LinkedList;

public final class bah extends a {
    public String jBB;
    public xu vIr;
    public String vLJ;
    public String vLW;
    public SKBuiltinBuffer_t vLz;

    public final int op(int i, Object... objArr) {
        AppMethodBeat.i(58914);
        b bVar;
        int ix;
        if (i == 0) {
            e.a.a.c.a aVar = (e.a.a.c.a) objArr[0];
            if (this.vLz == null) {
                bVar = new b("Not all required fields were included: RandomEncryKey");
                AppMethodBeat.o(58914);
                throw bVar;
            } else if (this.vIr == null) {
                bVar = new b("Not all required fields were included: CliPubECDHKey");
                AppMethodBeat.o(58914);
                throw bVar;
            } else {
                if (this.vLz != null) {
                    aVar.iy(1, this.vLz.computeSize());
                    this.vLz.writeFields(aVar);
                }
                if (this.vIr != null) {
                    aVar.iy(2, this.vIr.computeSize());
                    this.vIr.writeFields(aVar);
                }
                if (this.jBB != null) {
                    aVar.e(3, this.jBB);
                }
                if (this.vLJ != null) {
                    aVar.e(4, this.vLJ);
                }
                if (this.vLW != null) {
                    aVar.e(5, this.vLW);
                }
                AppMethodBeat.o(58914);
                return 0;
            }
        } else if (i == 1) {
            if (this.vLz != null) {
                ix = e.a.a.a.ix(1, this.vLz.computeSize()) + 0;
            } else {
                ix = 0;
            }
            if (this.vIr != null) {
                ix += e.a.a.a.ix(2, this.vIr.computeSize());
            }
            if (this.jBB != null) {
                ix += e.a.a.b.b.a.f(3, this.jBB);
            }
            if (this.vLJ != null) {
                ix += e.a.a.b.b.a.f(4, this.vLJ);
            }
            if (this.vLW != null) {
                ix += e.a.a.b.b.a.f(5, this.vLW);
            }
            AppMethodBeat.o(58914);
            return ix;
        } else if (i == 2) {
            e.a.a.a.a aVar2 = new e.a.a.a.a((byte[]) objArr[0], unknownTagHandler);
            for (ix = a.getNextFieldNumber(aVar2); ix > 0; ix = a.getNextFieldNumber(aVar2)) {
                if (!super.populateBuilderWithField(aVar2, this, ix)) {
                    aVar2.ems();
                }
            }
            if (this.vLz == null) {
                bVar = new b("Not all required fields were included: RandomEncryKey");
                AppMethodBeat.o(58914);
                throw bVar;
            } else if (this.vIr == null) {
                bVar = new b("Not all required fields were included: CliPubECDHKey");
                AppMethodBeat.o(58914);
                throw bVar;
            } else {
                AppMethodBeat.o(58914);
                return 0;
            }
        } else if (i == 3) {
            e.a.a.a.a aVar3 = (e.a.a.a.a) objArr[0];
            bah bah = (bah) objArr[1];
            int intValue = ((Integer) objArr[2]).intValue();
            LinkedList Vh;
            int size;
            byte[] bArr;
            e.a.a.a.a aVar4;
            boolean z;
            switch (intValue) {
                case 1:
                    Vh = aVar3.Vh(intValue);
                    size = Vh.size();
                    for (intValue = 0; intValue < size; intValue++) {
                        bArr = (byte[]) Vh.get(intValue);
                        SKBuiltinBuffer_t sKBuiltinBuffer_t = new SKBuiltinBuffer_t();
                        aVar4 = new e.a.a.a.a(bArr, unknownTagHandler);
                        for (z = true; z; z = sKBuiltinBuffer_t.populateBuilderWithField(aVar4, sKBuiltinBuffer_t, a.getNextFieldNumber(aVar4))) {
                        }
                        bah.vLz = sKBuiltinBuffer_t;
                    }
                    AppMethodBeat.o(58914);
                    return 0;
                case 2:
                    Vh = aVar3.Vh(intValue);
                    size = Vh.size();
                    for (intValue = 0; intValue < size; intValue++) {
                        bArr = (byte[]) Vh.get(intValue);
                        xu xuVar = new xu();
                        aVar4 = new e.a.a.a.a(bArr, unknownTagHandler);
                        for (z = true; z; z = xuVar.populateBuilderWithField(aVar4, xuVar, a.getNextFieldNumber(aVar4))) {
                        }
                        bah.vIr = xuVar;
                    }
                    AppMethodBeat.o(58914);
                    return 0;
                case 3:
                    bah.jBB = aVar3.BTU.readString();
                    AppMethodBeat.o(58914);
                    return 0;
                case 4:
                    bah.vLJ = aVar3.BTU.readString();
                    AppMethodBeat.o(58914);
                    return 0;
                case 5:
                    bah.vLW = aVar3.BTU.readString();
                    AppMethodBeat.o(58914);
                    return 0;
                default:
                    AppMethodBeat.o(58914);
                    return -1;
            }
        } else {
            AppMethodBeat.o(58914);
            return -1;
        }
    }
}
