                ...
                try {
                    if (IncrementComponentManager.a((Context) tinkerApplication, shareSecurityCheck)) {
                        ...
                        if (VERSION.SDK_INT < 27) {
                            HandlerMessageInterceptor handlerMessageInterceptor = new HandlerMessageInterceptor(jf(tinkerApplication), new MHMessageHandler(tinkerApplication));
                            ADm = handlerMessageInterceptor;
                            handlerMessageInterceptor.dSn();
                        } else {
                            TinkerHackInstrumentation jg = TinkerHackInstrumentation.jg(tinkerApplication);
                            ADn = jg;
                            jg.dSn();
                        }
                        ABB = true;
