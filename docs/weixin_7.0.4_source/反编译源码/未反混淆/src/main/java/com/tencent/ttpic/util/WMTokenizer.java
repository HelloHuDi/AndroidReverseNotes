...
public class WMTokenizer {
    ...
    public void doLayout() {
            ...
            while (true) {
                ...
                if (it.hasNext()) {
                    ...
                    switch (this.mAlignMode) {
                        case 1:
                            token.y = (((float) this.mHeight) - token.height) / 2.0f;
                            break;
                        case 3:
                            token.y = 0.0f;
                            break;
                        case 4:
                            token.x = ((float) this.mHeight) - token.height;
                            break;
                        default:
                            break;
                    }
                ...
                if (it.hasNext()) {
                    ...
                    switch (this.mAlignMode) {
                        case 0:
                            ...
                            break;
                        case 1:
                            token.x = (((float) this.mWidth) - token.width) / 2.0f;
                            break;
                        case 2:
                            token.x = ((float) this.mWidth) - token.width;
                            break;
                        default:
                            break;
                    }
        ...
        while (it.hasNext()) {
            token = (Token) it.next();
            this.mTextRect.left = this.mTextRect.left <= token.x ? this.mTextRect.left : token.x;
            this.mTextRect.right = this.mTextRect.right >= token.x + token.width ? this.mTextRect.right : token.x + token.width;
            this.mTextRect.top = this.mTextRect.top <= token.y ? this.mTextRect.top : token.y;
            this.mTextRect.bottom = this.mTextRect.bottom >= token.y + token.height ? this.mTextRect.bottom : token.height + token.y;
        }
        AppMethodBeat.o(84301);
    }
