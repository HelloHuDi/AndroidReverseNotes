package p000a.p010i.p011b.p012a.p049e;

import com.tencent.matrix.trace.core.AppMethodBeat;
import java.util.Collection;
import p000a.C0220l;
import p000a.p001a.C17115v;
import p000a.p005f.p006a.C17126b;
import p000a.p005f.p007b.C25052j;
import p000a.p010i.p011b.p012a.p015c.p023d.p024a.p029e.C0060t;
import p000a.p010i.p011b.p012a.p015c.p023d.p024a.p029e.C41408a;
import p000a.p010i.p011b.p012a.p015c.p023d.p024a.p029e.C44635g;
import p000a.p010i.p011b.p012a.p015c.p778f.C37022f;
import p000a.p010i.p011b.p012a.p015c.p778f.C8174b;

@C0220l(dWo = {1, 1, 13}, dWp = {"\u0000Z\n\u0002\u0018\u0002\n\u0002\u0018\u0002\n\u0002\u0018\u0002\n\u0000\n\u0002\u0018\u0002\n\u0002\b\u0002\n\u0002\u0010 \n\u0002\u0018\u0002\n\u0002\b\u0005\n\u0002\u0010\u000b\n\u0002\b\u0002\n\u0002\u0010\u001e\n\u0002\b\u0004\n\u0002\u0010\u0000\n\u0000\n\u0002\u0010\u0001\n\u0000\n\u0002\u0018\u0002\n\u0000\n\u0002\u0018\u0002\n\u0002\u0018\u0002\n\u0000\n\u0002\u0010\b\n\u0000\n\u0002\u0010\u000e\n\u0000\u0018\u00002\u00020\u00012\u00020\u0002B\r\u0012\u0006\u0010\u0003\u001a\u00020\u0004¢\u0006\u0002\u0010\u0005J\u0013\u0010\u0014\u001a\u00020\u000e2\b\u0010\u0015\u001a\u0004\u0018\u00010\u0016H\u0002J\u0012\u0010\u0017\u001a\u0004\u0018\u00010\u00182\u0006\u0010\u0003\u001a\u00020\u0004H\u0016J\"\u0010\u0019\u001a\b\u0012\u0004\u0012\u00020\u001a0\u00112\u0012\u0010\u001b\u001a\u000e\u0012\u0004\u0012\u00020\u001d\u0012\u0004\u0012\u00020\u000e0\u001cH\u0016J\b\u0010\u001e\u001a\u00020\u001fH\u0016J\b\u0010 \u001a\u00020!H\u0016R\u001a\u0010\u0006\u001a\b\u0012\u0004\u0012\u00020\b0\u00078VX\u0004¢\u0006\u0006\u001a\u0004\b\t\u0010\nR\u0014\u0010\u0003\u001a\u00020\u0004X\u0004¢\u0006\b\n\u0000\u001a\u0004\b\u000b\u0010\fR\u0014\u0010\r\u001a\u00020\u000e8VX\u0004¢\u0006\u0006\u001a\u0004\b\r\u0010\u000fR\u001a\u0010\u0010\u001a\b\u0012\u0004\u0012\u00020\u00020\u00118VX\u0004¢\u0006\u0006\u001a\u0004\b\u0012\u0010\u0013¨\u0006\""}, dWq = {"Lkotlin/reflect/jvm/internal/structure/ReflectJavaPackage;", "Lkotlin/reflect/jvm/internal/structure/ReflectJavaElement;", "Lkotlin/reflect/jvm/internal/impl/load/java/structure/JavaPackage;", "fqName", "Lkotlin/reflect/jvm/internal/impl/name/FqName;", "(Lorg/jetbrains/kotlin/name/FqName;)V", "annotations", "", "Lkotlin/reflect/jvm/internal/impl/load/java/structure/JavaAnnotation;", "getAnnotations", "()Ljava/util/List;", "getFqName", "()Lorg/jetbrains/kotlin/name/FqName;", "isDeprecatedInJavaDoc", "", "()Z", "subPackages", "", "getSubPackages", "()Ljava/util/Collection;", "equals", "other", "", "findAnnotation", "", "getClasses", "Lkotlin/reflect/jvm/internal/impl/load/java/structure/JavaClass;", "nameFilter", "Lkotlin/Function1;", "Lkotlin/reflect/jvm/internal/impl/name/Name;", "hashCode", "", "toString", "", "descriptors.runtime"})
/* renamed from: a.i.b.a.e.u */
public final class C36661u extends C31792n implements C0060t {
    private final C8174b BfP;

    public C36661u(C8174b c8174b) {
        C25052j.m39376p(c8174b, "fqName");
        AppMethodBeat.m2504i(123113);
        this.BfP = c8174b;
        AppMethodBeat.m2505o(123113);
    }

    public final C8174b dZF() {
        return this.BfP;
    }

    /* renamed from: m */
    public final /* synthetic */ C41408a mo18057m(C8174b c8174b) {
        AppMethodBeat.m2504i(123109);
        C25052j.m39376p(c8174b, "fqName");
        AppMethodBeat.m2505o(123109);
        return null;
    }

    public final boolean equals(Object obj) {
        AppMethodBeat.m2504i(123110);
        if ((obj instanceof C36661u) && C25052j.m39373j(this.BfP, ((C36661u) obj).BfP)) {
            AppMethodBeat.m2505o(123110);
            return true;
        }
        AppMethodBeat.m2505o(123110);
        return false;
    }

    public final String toString() {
        AppMethodBeat.m2504i(123112);
        String str = getClass().getName() + ": " + this.BfP;
        AppMethodBeat.m2505o(123112);
        return str;
    }

    /* renamed from: d */
    public final Collection<C44635g> mo105d(C17126b<? super C37022f, Boolean> c17126b) {
        AppMethodBeat.m2504i(123108);
        C25052j.m39376p(c17126b, "nameFilter");
        Collection collection = C17115v.AUP;
        AppMethodBeat.m2505o(123108);
        return collection;
    }

    public final Collection<C0060t> ebZ() {
        return C17115v.AUP;
    }

    public final /* bridge */ /* synthetic */ Collection ebC() {
        return C17115v.AUP;
    }

    public final int hashCode() {
        AppMethodBeat.m2504i(123111);
        int hashCode = this.BfP.hashCode();
        AppMethodBeat.m2505o(123111);
        return hashCode;
    }
}
