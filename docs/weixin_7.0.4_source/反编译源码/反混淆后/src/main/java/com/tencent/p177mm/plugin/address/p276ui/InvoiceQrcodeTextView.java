package com.tencent.p177mm.plugin.address.p276ui;

import android.content.ClipboardManager;
import android.content.Context;
import android.content.res.TypedArray;
import android.graphics.Rect;
import android.text.SpannableString;
import android.text.TextUtils.TruncateAt;
import android.text.style.BackgroundColorSpan;
import android.util.AttributeSet;
import android.view.ContextMenu;
import android.view.ContextMenu.ContextMenuInfo;
import android.view.LayoutInflater;
import android.view.MenuItem;
import android.view.MotionEvent;
import android.view.View;
import android.view.View.OnClickListener;
import android.view.View.OnCreateContextMenuListener;
import android.view.View.OnFocusChangeListener;
import android.view.View.OnLongClickListener;
import android.view.inputmethod.InputMethodManager;
import android.widget.ImageView;
import android.widget.PopupWindow.OnDismissListener;
import android.widget.RelativeLayout;
import android.widget.TextView;
import com.tencent.matrix.trace.core.AppMethodBeat;
import com.tencent.p177mm.C25738R;
import com.tencent.p177mm.p208bz.C1338a;
import com.tencent.p177mm.p612ui.base.C30379h;
import com.tencent.p177mm.p612ui.base.C40671n.C5279d;
import com.tencent.p177mm.p612ui.widget.p1095b.C24088a;
import com.tencent.p177mm.sdk.platformtools.C4990ab;
import com.tencent.p177mm.sdk.platformtools.C4996ah;
import com.tencent.p177mm.sdk.platformtools.C5046bo;

/* renamed from: com.tencent.mm.plugin.address.ui.InvoiceQrcodeTextView */
public class InvoiceQrcodeTextView extends RelativeLayout implements OnFocusChangeListener {
    private int background;
    private OnFocusChangeListener gJD;
    private TextView gJE;
    private ImageView gJG;
    private String gJH;
    private String gJI;
    private int gJJ;
    private int gJK;
    public boolean gJL;
    private int gJM;
    public boolean gJN;
    private int gJO;
    private int gJP;
    private boolean gJQ;
    private OnClickListener gJR;
    private String gJS;
    public boolean gJU;
    private int gJY;
    public boolean gJZ;
    private C18987a gKo;
    private C26703c gKp;
    private C26698b gKq;
    TextView gKr;
    private int gravity;
    private int imeOptions;
    private int inputType;

    /* renamed from: com.tencent.mm.plugin.address.ui.InvoiceQrcodeTextView$a */
    public interface C18987a {
    }

    /* renamed from: com.tencent.mm.plugin.address.ui.InvoiceQrcodeTextView$2 */
    class C266972 implements OnClickListener {
        C266972() {
        }

        public final void onClick(View view) {
            AppMethodBeat.m2504i(16885);
            if (InvoiceQrcodeTextView.this.gJG.getVisibility() == 0) {
                if (InvoiceQrcodeTextView.this.gJL && InvoiceQrcodeTextView.this.gJJ != 2 && !C5046bo.isNullOrNil(InvoiceQrcodeTextView.this.getText())) {
                    InvoiceQrcodeTextView.this.gKr.setText("");
                    InvoiceQrcodeTextView.m42532a(InvoiceQrcodeTextView.this, InvoiceQrcodeTextView.this.gKr.isFocused());
                    AppMethodBeat.m2505o(16885);
                    return;
                } else if (InvoiceQrcodeTextView.this.gKo != null) {
                    InvoiceQrcodeTextView.this.gKo;
                }
            }
            AppMethodBeat.m2505o(16885);
        }
    }

    /* renamed from: com.tencent.mm.plugin.address.ui.InvoiceQrcodeTextView$b */
    public interface C26698b {
    }

    /* renamed from: com.tencent.mm.plugin.address.ui.InvoiceQrcodeTextView$1 */
    class C266991 implements OnLongClickListener {
        C266991() {
        }

        public final boolean onLongClick(View view) {
            AppMethodBeat.m2504i(16884);
            final ClipboardManager clipboardManager = (ClipboardManager) C4996ah.getContext().getSystemService("clipboard");
            final TextView textView = (TextView) view;
            if (!(textView.getText() == null || clipboardManager == null)) {
                final CharSequence text = textView.getText();
                SpannableString spannableString = new SpannableString(text);
                spannableString.setSpan(new BackgroundColorSpan(textView.getContext().getResources().getColor(C25738R.color.f12117sz)), 0, text.length(), 33);
                textView.setText(spannableString);
                C24088a c24088a = new C24088a(textView.getContext(), textView);
                c24088a.zRZ = new OnCreateContextMenuListener() {
                    public final void onCreateContextMenu(ContextMenu contextMenu, View view, ContextMenuInfo contextMenuInfo) {
                        AppMethodBeat.m2504i(16881);
                        contextMenu.add(textView.getContext().getString(C25738R.string.f9082oy));
                        AppMethodBeat.m2505o(16881);
                    }
                };
                c24088a.rBm = new C5279d() {
                    public final void onMMMenuItemSelected(MenuItem menuItem, int i) {
                        AppMethodBeat.m2504i(16882);
                        if (i == 0) {
                            clipboardManager.setText(textView.getText().toString());
                            C30379h.m48464bP(textView.getContext(), textView.getContext().getString(C25738R.string.f9083oz));
                        }
                        AppMethodBeat.m2505o(16882);
                    }
                };
                c24088a.zHM = new OnDismissListener() {
                    public final void onDismiss() {
                        AppMethodBeat.m2504i(16883);
                        textView.setText(text);
                        AppMethodBeat.m2505o(16883);
                    }
                };
                c24088a.mo22431dn(0, 0);
            }
            AppMethodBeat.m2505o(16884);
            return false;
        }
    }

    /* renamed from: com.tencent.mm.plugin.address.ui.InvoiceQrcodeTextView$c */
    public interface C26703c {
    }

    /* renamed from: a */
    static /* synthetic */ void m42532a(InvoiceQrcodeTextView invoiceQrcodeTextView, boolean z) {
        AppMethodBeat.m2504i(16907);
        invoiceQrcodeTextView.m42536dr(z);
        AppMethodBeat.m2505o(16907);
    }

    public InvoiceQrcodeTextView(Context context, AttributeSet attributeSet, int i) {
        super(context, attributeSet);
        AppMethodBeat.m2504i(16886);
        this.gJU = false;
        this.gJH = "";
        this.gJI = "";
        this.inputType = 1;
        this.gJY = 0;
        this.gravity = 19;
        this.gJJ = -1;
        this.background = -1;
        this.gJK = -1;
        this.gJL = true;
        this.gJZ = true;
        this.gJN = false;
        this.gJO = 0;
        this.gJP = 100;
        this.gJQ = true;
        this.gJR = new C266972();
        this.gJS = null;
        TypedArray obtainStyledAttributes = context.obtainStyledAttributes(attributeSet, C25738R.C8899a.InvoiceEditView, i, 0);
        int resourceId = obtainStyledAttributes.getResourceId(5, 0);
        if (resourceId != 0) {
            this.gJH = context.getString(resourceId);
        }
        resourceId = obtainStyledAttributes.getResourceId(6, 0);
        if (resourceId != 0) {
            this.gJI = context.getString(resourceId);
        }
        this.inputType = obtainStyledAttributes.getInteger(3, 1);
        this.gJJ = obtainStyledAttributes.getInteger(7, 0);
        this.gJL = obtainStyledAttributes.getBoolean(10, true);
        this.gravity = obtainStyledAttributes.getInt(0, 19);
        this.imeOptions = obtainStyledAttributes.getInteger(4, 5);
        this.background = obtainStyledAttributes.getResourceId(1, C25738R.drawable.a8g);
        this.gJM = obtainStyledAttributes.getResourceId(12, -1);
        this.gJK = obtainStyledAttributes.getResourceId(9, C25738R.drawable.a8g);
        this.gJQ = obtainStyledAttributes.getBoolean(13, true);
        obtainStyledAttributes.recycle();
        View inflate = LayoutInflater.from(context).inflate(2130969878, this, true);
        this.gKr = (TextView) inflate.findViewById(2131821052);
        this.gKr.setTextSize(0, (float) C1338a.m2856al(context, C25738R.dimen.f9980m5));
        this.gJE = (TextView) inflate.findViewById(2131821051);
        this.gJG = (ImageView) inflate.findViewById(2131821053);
        this.gJG.setOnClickListener(this.gJR);
        this.gKr.setImeOptions(this.imeOptions);
        if (!C5046bo.isNullOrNil(this.gJH)) {
            this.gKr.setHint(this.gJH);
        }
        if (!C5046bo.isNullOrNil(this.gJI)) {
            this.gJE.setText(this.gJI);
        }
        Rect rect = new Rect();
        InvoiceQrcodeTextView.m42537i(this.gKr, rect);
        InvoiceQrcodeTextView.m42538j(this.gKr, rect);
        setPadding(C1338a.fromDPToPix(getContext(), 8), getPaddingTop(), getPaddingRight(), getPaddingBottom());
        if (this.gJM != -1) {
            this.gJG.setImageResource(this.gJM);
        }
        if (!this.gJQ) {
            this.gKr.setSingleLine(false);
        }
        this.gKr.setOnLongClickListener(new C266991());
        AppMethodBeat.m2505o(16886);
    }

    public InvoiceQrcodeTextView(Context context, AttributeSet attributeSet) {
        this(context, attributeSet, 0);
    }

    public final void asd() {
        AppMethodBeat.m2504i(16887);
        setBackgroundResource(0);
        this.gJE.setTextColor(getResources().getColor(C25738R.color.f12296z6));
        this.gKr.setTextColor(getResources().getColor(C25738R.color.f11782h4));
        this.gKr.setInputType(0);
        this.gKr.clearFocus();
        this.gKr.setSingleLine(false);
        ((InputMethodManager) getContext().getSystemService("input_method")).hideSoftInputFromWindow(this.gKr.getWindowToken(), 0);
        AppMethodBeat.m2505o(16887);
    }

    public void setOnInputValidChangeListener(C26703c c26703c) {
        this.gKp = c26703c;
    }

    public void setOnInputInvoiceTypeChangeListener(C26698b c26698b) {
        this.gKq = c26698b;
    }

    public String getText() {
        AppMethodBeat.m2504i(16888);
        String charSequence = this.gKr.getText().toString();
        AppMethodBeat.m2505o(16888);
        return charSequence;
    }

    public void setEllipsize(TruncateAt truncateAt) {
        AppMethodBeat.m2504i(16889);
        this.gKr.setEllipsize(truncateAt);
        AppMethodBeat.m2505o(16889);
    }

    public void setEnabled(boolean z) {
        AppMethodBeat.m2504i(16890);
        super.setEnabled(z);
        this.gJL = z;
        this.gJG.setEnabled(true);
        AppMethodBeat.m2505o(16890);
    }

    public boolean onInterceptTouchEvent(MotionEvent motionEvent) {
        AppMethodBeat.m2504i(16891);
        if (!this.gJL) {
            boolean z;
            if (this.gJG.getVisibility() == 0) {
                z = true;
            } else {
                z = false;
            }
            z = z ? getValidRectOfInfoIv().contains((int) motionEvent.getX(), (int) motionEvent.getY()) : false;
            if (!z) {
                AppMethodBeat.m2505o(16891);
                return true;
            }
        }
        AppMethodBeat.m2505o(16891);
        return false;
    }

    public void setOnClickListener(OnClickListener onClickListener) {
        AppMethodBeat.m2504i(16892);
        super.setOnClickListener(onClickListener);
        AppMethodBeat.m2505o(16892);
    }

    /* renamed from: dr */
    private void m42536dr(boolean z) {
        AppMethodBeat.m2504i(16893);
        if (!this.gJL || C5046bo.isNullOrNil(getText())) {
            switch (this.gJJ) {
                case 0:
                case 1:
                case 4:
                    this.gJG.setVisibility(8);
                    AppMethodBeat.m2505o(16893);
                    return;
                case 2:
                    this.gJG.setVisibility(0);
                    this.gJG.setContentDescription(getContext().getString(C25738R.string.f8739dm));
                    AppMethodBeat.m2505o(16893);
                    return;
                case 3:
                    this.gJG.setVisibility(0);
                    this.gJG.setContentDescription(getContext().getString(C25738R.string.f8760e8));
                    AppMethodBeat.m2505o(16893);
                    return;
                default:
                    this.gJG.setVisibility(8);
                    AppMethodBeat.m2505o(16893);
                    return;
            }
        }
        this.gJG.setImageResource(C25738R.drawable.f6801r8);
        this.gJG.setContentDescription(getContext().getString(C25738R.string.aqw));
        switch (this.gJJ) {
            case 0:
            case 1:
            case 4:
            case 5:
                if (z) {
                    this.gJG.setVisibility(0);
                    AppMethodBeat.m2505o(16893);
                    return;
                }
                this.gJG.setVisibility(8);
                AppMethodBeat.m2505o(16893);
                return;
            case 2:
                this.gJG.setVisibility(0);
                this.gJG.setContentDescription(getContext().getString(C25738R.string.f8739dm));
                AppMethodBeat.m2505o(16893);
                return;
            case 3:
                this.gJG.setVisibility(0);
                this.gJG.setContentDescription(getContext().getString(C25738R.string.f8760e8));
                AppMethodBeat.m2505o(16893);
                return;
            default:
                this.gJG.setVisibility(8);
                AppMethodBeat.m2505o(16893);
                return;
        }
    }

    public void setOnFocusChangeListener(OnFocusChangeListener onFocusChangeListener) {
        AppMethodBeat.m2504i(16894);
        super.setOnFocusChangeListener(onFocusChangeListener);
        this.gJD = onFocusChangeListener;
        AppMethodBeat.m2505o(16894);
    }

    public void setInfoIvOnClickListener(C18987a c18987a) {
        this.gKo = c18987a;
    }

    public void onFocusChange(View view, boolean z) {
        AppMethodBeat.m2504i(16895);
        if (this.gJD != null) {
            this.gJD.onFocusChange(this, z);
        }
        C4990ab.m7410d("MicroMsg.InvoiceEditView", "View:" + this.gJI + ", editType:" + this.gJJ + " onFocusChange to " + z);
        if (this.gJN) {
            this.gJE.setEnabled(true);
        } else {
            this.gJE.setEnabled(false);
        }
        if (view == this.gKr) {
            Rect rect = new Rect();
            InvoiceQrcodeTextView.m42537i(this, rect);
            if (z) {
                setBackgroundResource(C25738R.drawable.b77);
            } else {
                setBackgroundResource(C25738R.drawable.b78);
            }
            InvoiceQrcodeTextView.m42538j(this, rect);
        }
        m42536dr(z);
        AppMethodBeat.m2505o(16895);
    }

    public void setHintStr(String str) {
        AppMethodBeat.m2504i(16896);
        this.gKr.setHint(str);
        AppMethodBeat.m2505o(16896);
    }

    public void setTipStr(String str) {
        AppMethodBeat.m2504i(16897);
        this.gJE.setText(str);
        AppMethodBeat.m2505o(16897);
    }

    public void setValStr(String str) {
        AppMethodBeat.m2504i(16898);
        this.gKr.setText(str);
        this.gJS = str;
        AppMethodBeat.m2505o(16898);
    }

    public void setBankNumberValStr(String str) {
        CharSequence str2;
        AppMethodBeat.m2504i(16899);
        if (this.gJJ == 5) {
            String replace = str2.replace(" ", "");
            if (replace.length() >= 4) {
                StringBuilder stringBuilder = new StringBuilder();
                for (int i = 0; i < replace.length() / 4; i++) {
                    stringBuilder.append(replace.substring(i * 4, (i + 1) * 4)).append(" ");
                }
                str2 = stringBuilder.append(replace.substring((replace.length() / 4) * 4, replace.length())).toString();
            }
        }
        this.gKr.setText(str2);
        AppMethodBeat.m2505o(16899);
    }

    public void setEditBG(int i) {
        AppMethodBeat.m2504i(16900);
        if (this.gKr != null) {
            Rect rect = new Rect();
            InvoiceQrcodeTextView.m42537i(this.gKr, rect);
            this.gKr.setBackgroundResource(i);
            InvoiceQrcodeTextView.m42538j(this.gKr, rect);
        }
        AppMethodBeat.m2505o(16900);
    }

    public void setTipTextColor(int i) {
        AppMethodBeat.m2504i(16901);
        if (this.gJE != null) {
            this.gJE.setTextColor(i);
        }
        AppMethodBeat.m2505o(16901);
    }

    public void setImeOptions(int i) {
        AppMethodBeat.m2504i(16902);
        this.gKr.setImeOptions(i);
        AppMethodBeat.m2505o(16902);
    }

    public void setInfoIvVisible(int i) {
        AppMethodBeat.m2504i(16903);
        this.gJG.setVisibility(i);
        AppMethodBeat.m2505o(16903);
    }

    private Rect getValidRectOfInfoIv() {
        AppMethodBeat.m2504i(16904);
        Rect rect = new Rect();
        this.gJG.getHitRect(rect);
        rect.left -= 50;
        rect.right += 50;
        rect.top -= 25;
        rect.bottom += 25;
        AppMethodBeat.m2505o(16904);
        return rect;
    }

    /* renamed from: i */
    private static void m42537i(View view, Rect rect) {
        AppMethodBeat.m2504i(16905);
        rect.left = view.getPaddingLeft();
        rect.right = view.getPaddingRight();
        rect.top = view.getPaddingTop();
        rect.bottom = view.getPaddingBottom();
        AppMethodBeat.m2505o(16905);
    }

    /* renamed from: j */
    private static void m42538j(View view, Rect rect) {
        AppMethodBeat.m2504i(16906);
        view.setPadding(rect.left, rect.top, rect.right, rect.bottom);
        AppMethodBeat.m2505o(16906);
    }
}
