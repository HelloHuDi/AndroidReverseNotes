package com.tencent.p177mm.opensdk.openapi;

import android.content.Context;
import android.content.Intent;
import android.content.pm.PackageManager.NameNotFoundException;
import android.content.pm.Signature;
import com.tencent.matrix.trace.core.AppMethodBeat;
import com.tencent.p177mm.opensdk.constants.ConstantsAPI.Token;
import com.tencent.p177mm.opensdk.utils.Log;

/* renamed from: com.tencent.mm.opensdk.openapi.WXApiImplComm */
class WXApiImplComm {
    private static final String TAG = "MicroMsg.SDK.WXMsgImplComm";
    private static final String WX_APP_SIGNATURE = "";

    private WXApiImplComm() {
        AppMethodBeat.m2504i(128090);
        RuntimeException runtimeException = new RuntimeException(getClass().getSimpleName() + " should not be instantiated");
        AppMethodBeat.m2505o(128090);
        throw runtimeException;
    }

    public static boolean isIntentFromWx(Intent intent, String str) {
        AppMethodBeat.m2504i(128091);
        if (intent == null) {
            AppMethodBeat.m2505o(128091);
            return false;
        }
        String stringExtra = intent.getStringExtra(Token.WX_TOKEN_KEY);
        if (stringExtra == null || !stringExtra.equals(str)) {
            AppMethodBeat.m2505o(128091);
            return false;
        }
        AppMethodBeat.m2505o(128091);
        return true;
    }

    public static boolean validateAppSignature(Context context, Signature[] signatureArr, boolean z) {
        AppMethodBeat.m2504i(128093);
        if (z) {
            for (Signature toCharsString : signatureArr) {
                String toLowerCase = toCharsString.toCharsString().toLowerCase();
                Log.m4139d(TAG, "check signature:".concat(String.valueOf(toLowerCase)));
                if (toLowerCase.equals(WX_APP_SIGNATURE)) {
                    Log.m4139d(TAG, "pass");
                    AppMethodBeat.m2505o(128093);
                    return true;
                }
            }
            AppMethodBeat.m2505o(128093);
            return false;
        }
        Log.m4139d(TAG, "ignore wechat app signature validation");
        AppMethodBeat.m2505o(128093);
        return true;
    }

    public static boolean validateAppSignatureForPackage(Context context, String str, boolean z) {
        AppMethodBeat.m2504i(128092);
        if (z) {
            try {
                boolean validateAppSignature = WXApiImplComm.validateAppSignature(context, context.getPackageManager().getPackageInfo(str, 64).signatures, z);
                AppMethodBeat.m2505o(128092);
                return validateAppSignature;
            } catch (NameNotFoundException e) {
                AppMethodBeat.m2505o(128092);
                return false;
            }
        }
        Log.m4139d(TAG, "ignore wechat app signature validation");
        AppMethodBeat.m2505o(128092);
        return true;
    }
}
