...
abstract class BaseDecodeDataComponent {
    ...
    /* renamed from: com.tencent.qqmusic.mediaplayer.BaseDecodeDataComponent$1 */
    class C315061 implements WaitListener {
        C315061() {
        }

        public boolean keepWaiting() {
            AppMethodBeat.m2504i(104671);
            boolean isPaused = BaseDecodeDataComponent.this.isPaused();
            AppMethodBeat.m2505o(104671);
            return isPaused;
        }
    }
    ...
    public void refreshTimeAndNotify(int i) {
        ...
        if (this.mSignalControl.isWaiting()) {
            Logger.m71020d(TAG, axiliary("lock is Waiting, event: seek, doNotify"));
    ...
    public void play() {
        ...
        if (this.mSignalControl.isWaiting()) {
            Logger.m71020d(TAG, axiliary("lock is Waiting, event: play, doNotify"));
            this.mSignalControl.doNotify();
        }
    }
    ...
    public void stop() {
        ...
        if (this.mStateRunner.transfer(Integer.valueOf(6), Integer.valueOf(4), Integer.valueOf(5), Integer.valueOf(2)) && this.mSignalControl.isWaiting()) {
            Logger.m71020d(TAG, axiliary("lock is Waiting, event: stop, doNotify"));
            this.mSignalControl.doNotify();
        }
    }
    ...
    public void release() {
        if (this.mAudioTrack != null) {
            ...
            try {
                ...
                Logger.m71026i(TAG, "[run] failed to destroyAudioListeners!", th222);
            }
            this.mAudioTrack = null;
            Logger.m71020d(TAG, axiliary("finally release audioTrack"));
        }
    ...
    public void releaseNotify() {
        if (this.mSignalControl.isWaiting()) {
            Logger.m71020d(TAG, axiliary("lock is Waiting, event: release, doNotify"));
            this.mSignalControl.doNotify();
        }
    }
    ...
    public void setVolume(float f, float f2) {
        if (this.mAudioTrack != null) {
            try {
                ...
                Logger.m71022e(TAG, C27067l.NAME, e);
            }
        }
    }
        ...
        if (iAudioListener.isTerminal()) {
            synchronized (this.mTerminalAudioEffectList) {
                if (!this.mTerminalAudioEffectList.contains(iAudioListener)) {
                    ...
                    Logger.m71025i(TAG, "[addAudioListener] terminal audio added: ".concat(String.valueOf(iAudioListener)));
                }
            }
        } else {
            synchronized (this.audioEffects) {
                if (!this.audioEffects.contains(iAudioListener)) {
                    ...
                    Logger.m71025i(TAG, "[addAudioListener] audio added: ".concat(String.valueOf(iAudioListener)));
                }
            }
        }
        if (this.mInformation == null || this.mInformation.getPlaySample() <= MAX_PLAY_SAMPLE_RATE || this.mInformation.getChannels() <= 0) {
            Logger.m71025i(TAG, "[addAudioListener] audio information not ready. init will be delayed.");
            return;
        }
        long onPlayerReady;
        try {
            ...
            Logger.m71022e(TAG, "[addAudioListener] failed to init audio: ".concat(String.valueOf(iAudioListener)), th);
            onPlayerReady = MAX_PLAY_SAMPLE_RATE;
        }
        if (onPlayerReady != MAX_PLAY_SAMPLE_RATE) {
            Logger.m71023e(TAG, "[addAudioListener] failed to init audio %s, ret: %d", iAudioListener, Long.valueOf(onPlayerReady));
        }
    }
    ...
    public void removeAudioListener(IAudioListener iAudioListener) {
        synchronized (this.audioEffects) {
            if (this.audioEffects.remove(iAudioListener)) {
                Logger.m71025i(TAG, "[removeAudioListener] audio removed: ".concat(String.valueOf(iAudioListener)));
            }
        }
        synchronized (this.mTerminalAudioEffectList) {
            if (this.mTerminalAudioEffectList.remove(iAudioListener)) {
                Logger.m71025i(TAG, "[removeAudioListener] terminal audio removed: ".concat(String.valueOf(iAudioListener)));
            }
        }
    }
    ...
    private static boolean processAudioListener(IAudioListener iAudioListener, BufferInfo bufferInfo, BufferInfo bufferInfo2) {
        try {
            ...
            Logger.m71022e(TAG, "[processAudioListener] failed. audio: ".concat(String.valueOf(iAudioListener)), th);
            return false;
        }
    }
    ...
    protected static int getAudioTrackPosition(long j, AudioTrack audioTrack) {
        ...
        if (audioTrack != null) {
            try {
                ...
                Logger.m71022e(TAG, "getAudioTrackPosition", e);
            }
        }
        return (int) j2;
    }
    ...
    public void doWaitForPaused() {
        this.mSignalControl.doWait(FaceGestureDetGLThread.BRIGHTNESS_DURATION, 5, new C315061());
    }
}
