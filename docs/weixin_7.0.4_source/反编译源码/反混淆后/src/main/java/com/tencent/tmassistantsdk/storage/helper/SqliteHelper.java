...
public abstract class SqliteHelper extends SQLiteOpenHelper {
    ...
    public void onOpen(SQLiteDatabase sQLiteDatabase) {
        ...
        TMLog.m60588i("sqliteHelper", " dbversion:" + version + " newVersion:" + getDBVersion());
        if (version == 0) {
            return;
        }
    ...
    private void createTable(SQLiteDatabase sQLiteDatabase) {
        TMLog.m60588i("sqliteHelper", "tables count:" + getTables().length);
        for (Class newInstance : getTables()) {
            try {
                ...
                TMLog.m60588i("sqliteHelper", "sql=".concat(String.valueOf(createTableSQL)));
                if (GlobalUtil.isDBExist(AstSDKDBHelper.DB_NAME)) {
                    TMLog.m60588i("sqliteHelper", "dataMovement");
        ...
        while (i < i2) {
            for (Class newInstance : getTables()) {
                try {
                    ...
                    TMLog.m60588i("sqliteHelper", " upgrade:".concat(String.valueOf(alterSQL)));
                    if (alterSQL != null) {
