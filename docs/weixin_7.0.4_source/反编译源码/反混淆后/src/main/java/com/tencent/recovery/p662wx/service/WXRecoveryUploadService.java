...
import com.tencent.recovery.p662wx.util.PByteArray;
...
public class WXRecoveryUploadService extends RecoveryUploadService {
    ...
    public boolean tryToUploadData() {
        ...
        List a = RecoveryReporter.m8725a((Context) this, "HandleStatus", RecoveryHandleItem.class);
        boolean uploadData = uploadData(a);
        if (a.size() == 0 || uploadData) {
            RecoveryReporter.m8727bX(this, "HandleStatus");
        }
        return uploadData;
    }
    ...
    public boolean uploadData(List<RecoveryHandleItem> list) {
        ...
        for (RecoveryHandleItem recoveryHandleItem : list) {
            try {
                String str2;
                jSONArray.put(new JSONObject().put("tag", UNIT_REPORT_TAG).put("info", recoveryHandleItem.key).put(OpenSDKTool4Assistant.EXTRA_UIN, recoveryHandleItem.eCq).put("deviceUUID", iB).put("time", Util.m8738nz(recoveryHandleItem.timestamp)).put("cver", recoveryHandleItem.clientVersion).put("processName", recoveryHandleItem.processName).put("phoneStatus", recoveryHandleItem.Arn));
                if (WXRecoveryUploadService.compareVersion(recoveryHandleItem.clientVersion, str) > 0) {
        ...
        RecoveryLog.m8712i(TAG, "upload result %b %d", Boolean.valueOf(pushData(jSONArray, str)), Integer.valueOf(list.size()));
        return pushData(jSONArray, str);
    }

    private boolean pushData(JSONArray jSONArray, String str) {
        try {
            String iH = WXUtil.m8762iH(this);
            ...
            String toLowerCase = EncryptUtil.m8747x(String.format("weixin#$()%d%d", new Object[]{Integer.decode(str), Integer.valueOf(bytes.length)}).getBytes()).toLowerCase();
            bytes = EncryptUtil.compress(bytes);
            PByteArray pByteArray = new PByteArray();
            EncryptUtil.m8745a(pByteArray, bytes, toLowerCase.getBytes());
            ...
            if (httpURLConnection.getResponseCode() != 200) {
                String convertStreamToString = WXRecoveryUploadService.convertStreamToString(httpURLConnection.getErrorStream());
                str2 = WXRecoveryUploadService.convertStreamToString(httpURLConnection.getInputStream());
                RecoveryLog.m8712i(TAG, "POST returned: %d %s %s %s", Integer.valueOf(httpURLConnection.getResponseCode()), str, convertStreamToString, str2);
                return false;
            }
            RecoveryLog.m8712i(TAG, "POST returned success %s", str);
            return true;
        } catch (Exception e) {
