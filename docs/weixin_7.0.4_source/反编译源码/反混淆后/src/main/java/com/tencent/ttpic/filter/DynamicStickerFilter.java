...
public class DynamicStickerFilter extends NormalVideoFilter {
    ...
    public void updatePositions(List<PointF> list, float[] fArr, float f) {
        ...
        addParam(new C25629b("texAnchor", pointF3.x - ((float) this.canvasCenter.x), pointF3.y - ((float) this.canvasCenter.y)));
        ...
        addParam(new C0933f("texScale", (float) (Math.sqrt(Math.pow((double) (pointF2.y - pointF4.y), 2.0d) + Math.pow((double) (pointF2.x - pointF4.x), 2.0d)) / ((double) this.item.scaleFactor))));
        if (this.item.support3D == 1) {
            addParam(new C25630d("texRotate", fArr[0], fArr[1], (-fArr[2]) - this.item.angle));
            AppMethodBeat.m2505o(82174);
            return;
        }
        addParam(new C25630d("texRotate", 0.0f, 0.0f, (-fArr[2]) - this.item.angle));
        AppMethodBeat.m2505o(82174);
    }

    /* Access modifiers changed, original: protected */
    public void updatePositions(List<PointF> list) {
        AppMethodBeat.m2504i(82175);
        super.updatePositions(list);
        if (BaseUtils.isEmpty(list) || !isValidItem(this.item)) {
            clearTextureParam();
            AppMethodBeat.m2505o(82175);
            return;
        }
        PointF pointF = (PointF) list.get(this.item.alignFacePoints[0]);
        ...
        addParam(new C0933f("texScale", (float) (Math.sqrt(Math.pow((double) (pointF4.y - pointF5.y), 2.0d) + Math.pow((double) (pointF4.x - pointF5.x), 2.0d)) / ((double) this.item.scaleFactor))));
        if (this.item.alignFacePoints.length > 1) {
            addParam(new C25630d("texRotate", 0.0f, 0.0f, ((float) Math.atan2((double) (pointF.y - pointF2.y), (double) (pointF.x - pointF2.x))) - this.item.angle));
        }
        AppMethodBeat.m2505o(82175);
    }
