package com.tencent.p177mm.plugin.wenote.model.p585a;

import com.tencent.p177mm.plugin.wenote.model.nativenote.manager.WXRTEditText;

/* renamed from: com.tencent.mm.plugin.wenote.model.a.c */
public abstract class C44001c {
    public String mnd = "";
    public boolean uNT = false;
    public int uNU = 0;
    public int uNV = -1;
    public WXRTEditText uNW;
    public WXRTEditText uNX;
    public WXRTEditText uNY;
    public boolean uNZ = false;
    public boolean uOa = false;
    public boolean uOb = false;
    public int uOc = 0;
    public int uOd = 0;

    public abstract String ddP();

    public int getType() {
        return 0;
    }

    public String ddQ() {
        return "";
    }
}
