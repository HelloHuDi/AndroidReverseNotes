...
public abstract class TMAssistantDownloadSDKClientBase implements ServiceConnection {
    ...
    public void onServiceDisconnected(ComponentName componentName) {
        TMLog.m60588i(TAG, "onServiceDisconnected,clientKey:" + this.mClientKey);
        synchronized (this) {
    ...
    public void onServiceConnected(ComponentName componentName, IBinder iBinder) {
        ...
        TMLog.m60588i(TAG, "onServiceConnected,clientKey:" + this.mClientKey + ",mServiceInterface:" + this.mServiceInterface + ",IBinder:" + iBinder + ",threadId:" + Thread.currentThread().getId());
        try {
            if (this.mServiceInterface != null && this.mServiceCallback != null) {
                registerServiceCallback();
            }
        } catch (RemoteException e) {
            TMLog.m60588i(TAG, "onServiceConnected, exception : " + e.getMessage());
            onDownloadSDKServiceInvalid();
        }
    }
    ...
    public synchronized boolean initTMAssistantDownloadSDK() {
        boolean z = false;
        synchronized (this) {
            TMLog.m60588i(TAG, "connectState = " + this.connectState);
            if (this.connectState != CONNTECTSTATE_INIT) {
                z = true;
            } else {
                TMLog.m60588i(TAG, "initTMAssistantDownloadSDK,clientKey:" + this.mClientKey + ",mServiceInterface:" + this.mServiceInterface + ",threadId:" + Thread.currentThread().getId());
                this.connectState = CONNTECTSTATE_INIT;
                if (this.mServiceInterface != null) {
                    ...
                    TMLog.m60588i(TAG, "initTMAssistantDownloadSDK bindResult:".concat(String.valueOf(z)));
                }
            }
        }
        return z;
    ...
    public synchronized void unInitTMAssistantDownloadSDK() {
        TMLog.m60588i(TAG, "unInitTMAssistantDownloadSDK,clientKey:" + this.mClientKey + ",mServiceInterface:" + this.mServiceInterface + ",threadId:" + Thread.currentThread().getId());
        if (!(this.mServiceInterface == null || this.mServiceCallback == null)) {
            try {
