...
public class CompositeGeneratedAdaptersObserver implements GenericLifecycleObserver {
    /* renamed from: cj */
    private final GeneratedAdapter[] f2019cj;

    CompositeGeneratedAdaptersObserver(GeneratedAdapter[] generatedAdapterArr) {
        this.f2019cj = generatedAdapterArr;
    }

    public void onStateChanged(LifecycleOwner lifecycleOwner, Event event) {
        int i = 0;
        MethodCallsLogger methodCallsLogger = new MethodCallsLogger();
        for (GeneratedAdapter callMethods : this.f2019cj) {
            callMethods.callMethods(lifecycleOwner, event, false, methodCallsLogger);
        }
        GeneratedAdapter[] generatedAdapterArr = this.f2019cj;
